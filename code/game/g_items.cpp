...
int Pickup_Holocron( gentity_t *ent, gentity_t *other )
{
	...
	if ( ( other->client->ps.forcePowersKnown & ( 1 << forcePower )) )
	{
		...
		if ( other->client->ps.forcePowerLevel[forcePower] >= Q3_INFINITE )
		{
			return 1;
		}
		else
		{
			...
			{
				other->client->ps.forcePowerLevel[forcePower] += 1;
			}

			if ( forcePower == FP_SABER_OFFENSE )
			{
				if ( !(other->client->ps.saberStylesKnown & ( 1 << SS_MEDIUM )) )
				{
					other->client->ps.saberAnimLevel = SS_MEDIUM;
				}

				for ( int ss = SS_NONE; ss < SS_NUM_SABER_STYLES; ss++ )
				{
					...
					case 1:
						break;
					case 2:
						if ( !(other->client->ps.saberStylesKnown & ( 1 << SS_STRONG )) )
						{
							other->client->ps.saberAnimLevel = SS_FAST;
						}
						break;
					case 3:
						switch ( ss )
						...
						case SS_MEDIUM:
						case SS_STRONG:
							if ( !(other->client->ps.saberStylesKnown & ( 1 << ss )) )
							{
								other->client->ps.saberAnimLevel = ss;
							}
							break;
						}
						break;
	...
	else
	{
		...
		if ( forcePower == FP_SABER_OFFENSE )
		{
			if ( other->client->ps.saberAnimLevel != SS_MEDIUM )
			{
				other->client->ps.saberAnimLevel = SS_MEDIUM;
			}
		}
...
{
	int	healAmount = MAX_BACTA_HEAL_AMOUNT;
	...
	{
		healAmount = (55 - (15 * g_spskill->integer));

		if ( healAmount > ent->client->ps.stats[STAT_MAX_HEALTH] )
		{
			healAmount = ent->client->ps.stats[STAT_MAX_HEALTH];
		}
	}
	else
	{
	...
	G_SoundOnEnt( ent, CHAN_ITEM, "sound/weapons/force/teamheal.mp3" );
...
void ItemUse_Cloak( gentity_t *ent ) 
{
...
void ItemUse_ForceGem( gentity_t *ent )
{
	float	maxForce = floor((float)ent->client->ps.forcePowerMax * 2.0f);
	int		forceAmount = 25;
	...
	if ( ent->client->ps.forcePower >= maxForce || 
		!ent->client->ps.inventory[INV_FORCE_GEM] )
	{
		return;
	}
	...
	if ( g_spskill->integer < 0 )
	{
		forceAmount = (55 - (15 * g_spskill->integer));
	...
	ent->client->ps.forcePower += forceAmount;

	if ( ent->client->ps.forcePower > maxForce )
	{
		ent->client->ps.forcePower = maxForce;
	}
	
	ent->client->ps.inventory[INV_FORCE_GEM]--;

	G_SoundOnEnt( ent, CHAN_ITEM, "sound/weapons/force/teamforce.mp3" );
}
