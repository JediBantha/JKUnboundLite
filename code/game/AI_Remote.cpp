...
void Remote_Hunt( qboolean visible, qboolean advance, qboolean retreat )
{
	...
	speed = REMOTE_FORWARD_BASE_SPEED + REMOTE_FORWARD_MULTIPLIER * G_DifficultyLimit();
	if ( retreat == qtrue )
	{
		speed *= -1;
	}
...
void Remote_Fire (void)
{
	...
	{
		velocity = floor((float)1000 * (1.0 + (0.1f * (g_spskill->integer - 2))));
	...
	missile = CreateMissile( NPC->currentOrigin, forward, velocity, 10000, NPC );
