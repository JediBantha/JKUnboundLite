		...
		if ( !hitDodged )
		{
			if ( render_impact )
			{
					...
					|| traceEnt->s.eType == ET_MOVER )
				{
					...
					{//hehe
						G_Damage( traceEnt, ent, ent, forwardVec, tr.endpos, 10, dflags, MOD_CONC_ALT, hitLoc );
						break;
					}
					G_Damage( traceEnt, ent, ent, forwardVec, tr.endpos, damage, dflags, MOD_CONC_ALT, hitLoc );
...
{//a fast rocket-like projectile
	...
	int		dflags	= (DAMAGE_EXTRA_KNOCKBACK|DAMAGE_HEAVY_WEAP_CLASS);
	...
	if (ent->s.number >= MAX_CLIENTS)
	{
		...
		{
			vel = floor((float)CONC_VELOCITY * (1.0f + (0.1f * (g_spskill->integer - 2))));
