...
//	}
	...
	*/
	...
	if ( ent->s.number != 0 )
	{
		...
		{
			vel = floor((float)ATST_MAIN_VEL * (1.0f + (0.1f * (g_spskill->integer - 2))));
...
//---------------------------------------------------------
{
	...
	float	vel = ATST_SIDE_ALT_VELOCITY;//ATST_SIDE_ALT_NPC_VELOCITY;
	...
	*/
	...
	if ( ent->s.number != 0 )
	{
		if ( g_spskill->integer > 2 )
		{
			vel = floor((float)ATST_SIDE_ALT_VELOCITY * (1.0f + (0.1f * (g_spskill->integer - 2))));
...
//---------------------------------------------------------
{
	...
	float	vel = ATST_SIDE_MAIN_VELOCITY;

	if ( ent->s.number != 0 )
	{
		if ( g_spskill->integer > 2 )
		{
			vel = floor((float)ATST_SIDE_MAIN_VELOCITY * (1.0f + (0.1f * (g_spskill->integer - 2))));
	...
	gentity_t *missile = CreateMissile( muzzle, forwardVec, vel, 10000, ent, qfalse );
