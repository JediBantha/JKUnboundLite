...
{
	{ "forceHeal",			"setForceHeal",			Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceJump",			"setForceJump",			Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceSpeed",			"setForceSpeed",		Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forcePush",			"setForcePush",			Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forcePull",			"setForcePull",			Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceMindTrick",		"setForceMindTrick",	Q3_INFINITE				},//FORCE_LEVEL_4
	{ "forceGrip",			"setForceGrip",			Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceLightning",		"setForceLightning",	Q3_INFINITE				},//FORCE_LEVEL_3
	{ "saberThrow",			"setSaberThrow",		Q3_INFINITE				},//FORCE_LEVEL_3
	{ "saberDefense",		"setSaberDefense",		Q3_INFINITE				},//FORCE_LEVEL_3	//SS_NUM_SABER_STYLES-1
	{ "saberOffense",		"setSaberOffense",		Q3_INFINITE				},//SS_NUM_SABER_STYLES-1
	{ "forceRage",			"setForceRage",			Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceProtect",		"setForceProtect",		Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceAbsorb",		"setForceAbsorb",		Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceDrain",			"setForceDrain",		Q3_INFINITE				},//FORCE_LEVEL_3
	{ "forceSight",			"setForceSight",		Q3_INFINITE				},//FORCE_LEVEL_3
};
...
static void Svcmd_SetForceLightSide_f(void)
{
...
static void Svcmd_SetForceNeutral_f(void)
{
	for ( int i = FP_HEAL; i < NUM_FORCE_POWERS; i++ )
	{
		switch ( i )
...
static void Svcmd_SetForceDarkSide_f(void)
{
	for ( int i = FP_HEAL; i < NUM_FORCE_POWERS; i++ )
	{
		switch ( i )
...
static void Svcmd_SetForceAll_f(void)
{
	...
	if( gi.argc() > 1 )
	{
		...
		{		
			if ( g_entities[0].client->ps.forcePowerLevel[FP_SABER_OFFENSE] <= 0 )
			{
				if ( g_entities[0].client->ps.saberStylesKnown )
				{
					g_entities[0].client->ps.saberStylesKnown &= ~i;

					if ( g_entities[0].client->ps.saberAnimLevel != SS_MEDIUM )
					{
						g_entities[0].client->ps.saberAnimLevel = SS_MEDIUM;
					}
				}
			}
			else
			{
				switch ( g_entities[0].client->ps.forcePowerLevel[FP_SABER_OFFENSE] )
				{
				case 1:
					g_entities[0].client->ps.saberStylesKnown |= ( 1 << SS_MEDIUM );
					break;
				case 2:
					g_entities[0].client->ps.saberStylesKnown |= ( 1 << SS_MEDIUM );
		
					if ( !(g_entities[0].client->ps.saberStylesKnown & ( 1 << SS_STRONG )) )
					{
				...
				case 4:
					...
					case SS_STRONG:
					case SS_TAVION:
						if ( !(g_entities[0].client->ps.saberStylesKnown & ( 1 << i )) )
						{
							g_entities[0].client->ps.saberStylesKnown |= (1 << i);
						}
						break;
...
static void Svcmd_SetSaberAll_f(void)
{
	for ( int forcePower = FP_FIRST; forcePower < NUM_FORCE_POWERS; forcePower++ )
	{
		...
		case FP_SABER_OFFENSE:
			Svcmd_ForceSetLevel_f( forcePower );
			break;
		default:
			break;
		}
	...
	if(gi.argc() == 1)
	{
		if (g_spskill->integer <= 0)
		{
			...
			case 0:
				...
				break;
			case -1:
				gi.Printf( S_COLOR_GREEN "Current Difficulty: Apprentice" S_COLOR_WHITE "\n" );
				break;
			case -2:
				gi.Printf( S_COLOR_GREEN "Current Difficulty: Initiate" S_COLOR_WHITE "\n" );
				break;
			case -3:
			default:
				gi.Printf( S_COLOR_GREEN "Current Difficulty: Hopeful" S_COLOR_WHITE "\n" );
				break;
			}
		}
		...
		else if (g_spskill->integer >= 2)
		{
			...
			if (handicap <= 100 && crosshairHint == 0)
			{
				...
				case 3:
					...
					break;
				case 4:
					gi.Printf( S_COLOR_GREEN "Current Difficulty: Jedi Lord" S_COLOR_WHITE "\n" );
					break;
				case 5:
				default:
					gi.Printf( S_COLOR_GREEN "Current Difficulty: Chosen One" S_COLOR_WHITE "\n" );
					break;
				}
			}
			else
		...
		{
			gi.Printf( S_COLOR_RED "Invalid difficulty cvar set! g_spskill (%i) [0-Infinite] is valid range only" S_COLOR_WHITE "\n", g_spskill->integer );
		}
	}
}
...
static svcmd_t svcmds[] = 
{
	...
	{ "setForceLightSide",			Svcmd_SetForceLightSide_f,					CMD_CHEAT },
	{ "setForceNeutral",			Svcmd_SetForceNeutral_f,					CMD_CHEAT },
	{ "setForceDarkSide",			Svcmd_SetForceDarkSide_f,					CMD_CHEAT },
