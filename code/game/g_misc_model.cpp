	...
	if ( ct )
	{
		for ( int i = 0; i < ct; i++ )
		{
			GunRackAddItem( 
				...
				ent->s.origin, 
				ent->s.angles, 
				Q_flrand(-1.0f, 1.0f) * 2, 
				( i - 1 ) * 9 + Q_flrand(-1.0f, 1.0f) * 2, 
				ofz[i] 
			);
		}
	}
...
{
// If BLASTER is checked...or nothing is checked then we'll do blasters
	if (( ent->spawnflags & RACK_BLASTER ) || 
		!(ent->spawnflags & ( RACK_BLASTER | RACK_METAL_BOLTS | RACK_ROCKETS | RACK_PWR_CELL )))
	{
...
void misc_model_cargo_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int dFlags, int hitLoc )
{
	int		flags;
	vec3_t	org, temp;
	gitem_t *health = NULL, *shields = NULL, *bacta = NULL, *batteries = NULL, *forceGem = NULL;
	...
	if (( flags & DROP_FORCE_GEM ))
	{
		forceGem = FindItem( "item_force_gem" );
		...
		if ( forceGem )
		{
			...
			LaunchItem( forceGem, temp, vec3_origin, NULL );
		}
	}
}
...
void SP_misc_model_cargo_small( gentity_t *ent )
{
	...
	if (( ent->spawnflags & DROP_FORCE_GEM ))
	{
		RegisterItem( FindItem( "item_force_gem" ));
	}

	G_SpawnInt( "health", "25", &ent->health );

	SetMiscModelDefaults( 
		ent, 
		useF_NULL, 
		"11", 
		CONTENTS_SOLID|CONTENTS_OPAQUE|CONTENTS_BODY|CONTENTS_MONSTERCLIP|CONTENTS_BOTCLIP, 
		0, 
		qtrue, 
		qfalse 
	);
