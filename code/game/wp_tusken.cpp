...
{
	vec3_t	start;
	float	velocity = TUSKEN_RIFLE_VEL;
	
	if ( ent->s.number != 0 )
	{
		if ( g_spskill->integer > 2 )
		{
			velocity = floor((float)TUSKEN_RIFLE_VEL * (1.0f + (0.1f * (g_spskill->integer - 2))));
	...
	gentity_t	*missile = CreateMissile( start, forwardVec, velocity, 10000, ent, qfalse );
