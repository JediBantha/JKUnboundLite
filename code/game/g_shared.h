...
//icarus forced moving.  is this still used?
	...
	int			redLightningDamageDebounce;
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(redLightningDamageDebounce);
		...
		saved_game.write<float>(hackingAngles);
		saved_game.write<int32_t>(jetPackToggleTime);
		saved_game.write<int32_t>(jetPackDebRecharge);
		saved_game.write<int32_t>(jetPackDebReduce);
		saved_game.write<int32_t>(cloakToggleTime);
		saved_game.write<int32_t>(cloakDebRecharge);
		saved_game.write<int32_t>(cloakDebReduce);
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(redLightningDamageDebounce);
		...
		saved_game.read<float>(hackingAngles);
		saved_game.read<int32_t>(jetPackToggleTime);
		saved_game.read<int32_t>(jetPackDebRecharge);
		saved_game.read<int32_t>(jetPackDebReduce);
		saved_game.read<int32_t>(cloakToggleTime);
		saved_game.read<int32_t>(cloakDebRecharge);
		saved_game.read<int32_t>(cloakDebReduce);
...
//FIXME: Are these being used anymore?
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(hackValue);
		saved_game.write<int32_t>(triggerFX);
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(hackValue);
		saved_game.read<int32_t>(triggerFX);
