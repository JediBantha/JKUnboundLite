...
void Wampa_Slash( int boltIndex, qboolean backhand )
{
	...
	int			i;
	vec3_t		boltOrg;
	int			min = (20 + (5 * G_DifficultyLimit())), max = (min + 10);//	10, 15 : 20, 30
	int			damage = (backhand) ? Q_irand((min / 2), (min + 5)) : Q_irand(min, max);
		...
		else
		{
			if ( NPC->client->ps.legsAnim == BOTH_HOLD_DROP )
			{
				...
				{//at least half a second into the anim
					...
					{
						TIMER_Set( NPC, "attacking", 1000+(Q_irand(500,1000)*(3-G_DifficultyLimit())) );
					}
				}
			}
				...
				else
				{
					...
					if ( NPC->client->ps.legsAnim == BOTH_HOLD_ATTACK )
					{
						if ( NPC->client->ps.legsAnimTimer )
						{
							...
							{
								int	min = (25 + (5 * G_DifficultyLimit())), max = (min + 15);

								G_Sound( NPC->activator, G_SoundIndex( "sound/chars/rancor/swipehit.wav" ) );
								G_Damage( NPC->activator, NPC, NPC, vec3_origin, NPC->activator->currentOrigin, Q_irand( min, max ), (DAMAGE_NO_KNOCKBACK|DAMAGE_NO_ARMOR), MOD_MELEE );
