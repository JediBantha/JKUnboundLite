...
void Rancor_DropVictim( gentity_t *self )
{
	...
	if ( self->activator )
	{
		...
		{//don't attack the player again for a bit
			TIMER_Set( self, "attackDebounce", Q_irand( 2000, 4000+((2-G_DifficultyLimit())*2000) ) );
		}
...
void Rancor_Swing( int boltIndex, qboolean tryGrab )
{
	...
	{//attacking a breakable brush
		...
		{//fuck, do an actual line trace, I guess...
			gi.trace( &trace, originUp, vec3_origin, vec3_origin, 
				boltOrg, NPC->s.number, CONTENTS_SOLID|CONTENTS_BODY, (EG2_Collision)0, 0 );
#ifndef FINAL_BUILD
			if ( g_bobaDebug->integer > 0 )
			{
			...
			*/
			...
			{//smack
				...
				*/
				...
				{
					G_Damage( radiusEnts[i], NPC, NPC, vec3_origin, boltOrg, 
						radiusEnts[i]->health+1000, (DAMAGE_NO_KNOCKBACK|DAMAGE_NO_PROTECTION), MOD_MELEE );
				}
				vec3_t angs;
...
void Rancor_Smash( void )
{
	...
	int			i;
	vec3_t		boltOrg;
	int			min = (10 + (5 * G_DifficultyLimit())), max = (min + 15);//10, 25
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( distSq <= radiusSquared )
		{
			...
			{//close enough to do damage, too
				...
				{//more damage
					min = (40 + (5 * G_DifficultyLimit()));

					G_Damage( radiusEnts[i], NPC, NPC, vec3_origin, radiusEnts[i]->currentOrigin, Q_irand( min, max ), DAMAGE_NO_KNOCKBACK, MOD_MELEE );
				}
				else
				{
					G_Damage( radiusEnts[i], NPC, NPC, vec3_origin, radiusEnts[i]->currentOrigin, Q_irand( min, max ), DAMAGE_NO_KNOCKBACK, MOD_MELEE );
...
void Rancor_Bite( void )
{
	gentity_t	*radiusEnts[ 128 ];
	int			numEnts;
	int			min = (15 + (5 * G_DifficultyLimit())), max = (min + 15);//15, 30
...
void Rancor_Attack( float distance, qboolean doCharge, qboolean aimAtBlockedEntity )
{
	int min = (25 + (5 * G_DifficultyLimit())), max = (min + 15);//25, 40
		...
		&& TIMER_Done( NPC, "attackDebounce" ) )
	{
		...
		{//charge
			if ( !Q_irand( 0, 3 ) )
			{
				...
				{//don't attack the player again for a bit
					TIMER_Set( NPC, "attackDebounce", NPC->client->ps.legsAnimTimer + Q_irand( 2000, 4000+((2-G_DifficultyLimit())*2000) ) );
				}
			}
			else if ( (NPC->spawnflags&SPF_RANCOR_MUTANT) )
			{//breath attack
	...
	if ( TIMER_Done2( NPC, "attack_dmg", qtrue ) )
	{
		...
		case BOTH_ATTACK1:
			if ( NPC->count == 1 && NPC->activator )
			{
				...
				{//more damage
					min = (55 + (5 * G_DifficultyLimit()));

					G_Damage( NPC->activator, NPC, NPC, vec3_origin, NPC->activator->currentOrigin, Q_irand( min, max ), DAMAGE_NO_KNOCKBACK, MOD_MELEE );
				}
				else
				{
					G_Damage( NPC->activator, NPC, NPC, vec3_origin, NPC->activator->currentOrigin, Q_irand( min, max ), DAMAGE_NO_KNOCKBACK, MOD_MELEE );
	...
	{//holding my enemy
		...
		{//just sniffing the guy
			...
			{//just about done, drop him
				...
				TIMER_Set( NPC, "attacking", NPC->client->ps.legsAnimTimer+(Q_irand(500,1000)*(3-G_DifficultyLimit())) );
			}
		}
		else
		{
				...
				*/
				{//if going to bite our victim, only victim can interrupt that anim
					if ( self->health > 100 || hitByRancor )
					{
						...
						TIMER_Set( self, "takingPain", self->client->ps.legsAnimTimer+Q_irand(0, 500*(2-G_DifficultyLimit())) );
...
{
	int			min = (10 + G_DifficultyLimit()), max = (min * 2);//10, 15
