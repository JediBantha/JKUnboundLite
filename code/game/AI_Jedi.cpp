	...
	if ( NPC->genericBolt1 != -1 )
	{
		...
		for ( int time = curTime-25; time <= curTime+25&&!hit; time += 25 )
		{
			...
			{//hit something
				...
				{//smack
					int dmg = Q_irand( 10, 20 )*(G_DifficultyLimit()+1);//NOTE: was 6-12
					//FIXME: debounce?
					//FIXME: do dismemberment

					G_Damage( traceEnt, NPC, NPC, vec3_origin, trace.endpos, dmg, DAMAGE_NO_KNOCKBACK, MOD_SCEPTER );//MOD_MELEE );
		...
		for ( i = 0; i < numEnts; i++ )
		{
			...
			{//must be a client
				...
				{//damage breakables within range, but not as much
					G_Damage( radiusEnts[i], NPC, NPC, vec3_origin, radiusEnts[i]->currentOrigin, 100, 0, MOD_SCEPTER_ALT );
				}
				continue;
			}
...
extern qboolean NPC_IsJediClass( gentity_t *ent );
void Jedi_CloakDetectionCheck( gentity_t *self )
{
	trace_t		tr;
	vec3_t		forward;
	gentity_t	*traceEnt;
	...
	for ( e = 0; e < numListedEntities; e++ )
	{
		...
		if ( !traceEnt->bmodel && !gi.inPVS( ent_org, self->client->ps.origin ) )
		{
			continue;
		}

		gi.trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, 
			ent_org, self->s.number, MASK_SHOT, (EG2_Collision)0, 0 );
		...
		if ( InFront( self->currentOrigin, traceEnt->client->renderInfo.eyePoint, traceEnt->client->ps.viewangles, 0.3f ) )
		{
			Jedi_Decloak( self );
		}
	}
...
void Jedi_CheckCloak( void )
{
	...
	if ( self && self->client )
	{
		if ( self->client->NPC_class == CLASS_SHADOWTROOPER
			&& Q_stricmpn("shadowtrooper", self->NPC_type, 13) == 0)
		{
		...
		else
		{
			if ( self->client->ps.SaberActive() ||
				...
				self->painDebounceTime > level.time )
			{
				if ( self->client->cloakToggleTime < level.time 
					&& self->client->ps.SaberLength() > 0 )
				{
					Jedi_Decloak( self );
				}
			}
...
static void Jedi_AdjustSaberAnimLevel( gentity_t *self, int newLevel )
{
	...
	if ( d_JediAI->integer )
	{
		...
		{
		case SS_FAST:
			gi.Printf( S_COLOR_BLUE"%s Saber Attack Set: fast\n", self->NPC_type );
			break;
		case SS_MEDIUM:
		...
		case SS_STRONG:
			...
			break;
		case SS_DESANN:
			gi.Printf( S_COLOR_MAGENTA"%s Saber Attack Set: desann\n", self->NPC_type );
			break;
		case SS_TAVION:
			gi.Printf( S_COLOR_GREEN"%s Saber Attack Set: tavion\n", self->NPC_type );
			break;
		case SS_DUAL:
			gi.Printf( S_COLOR_YELLOW"%s Saber Attack Set: dual\n", self->NPC_type );
			break;
		case SS_STAFF:
			gi.Printf( S_COLOR_YELLOW"%s Saber Attack Set: staff\n", self->NPC_type );
			break;
		}
	}
}
	...
	*/
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're being gripped, rush them!
			...
			Q_irand( 0, 6 ) < G_DifficultyLimit() && //more likely on harder diff
		...
		{//They're throwing their saber, grip them!
			...
			//grip
			TIMER_Set( NPC, "gripping", Q_irand(3000, 5000) );
			TIMER_Set( NPC, "attackDelay", 3000 );
		}
		else
		{
			if ( NPC->enemy 
				&& NPC->enemy->client 
				&& (NPC->enemy->client->ps.forcePowersActive&(1<<FP_GRIP)) )
			...
			else
			{
				...
				{//hmm, bosses do this less against player
					chanceScale = 8 - G_DifficultyLimit()*2;
				}
				...
				{//else, randomly try some kind of attack every now and then
						...
						&& (!Q_irand( 0, 1 ) || NPC->s.weapon != WP_SABER) )
					{
							...
							&& (((NPCInfo->scriptFlags&SCF_DONT_FIRE)&&Q_stricmp("cultist_lightning",NPC->NPC_type)) || Q_irand( 0, 1 )) )
						{
							...
							{
								NPC->client->ps.weaponTime = Q_irand( 1000, 3000+(G_DifficultyLimit()*500) );
...
int Jedi_ReCalcParryTime( gentity_t *self, evasionType_t evasionType )
{
	...
	if ( !self->s.number )
	{//player
		int	parryTime = parryDebounce[self->client->ps.forcePowerLevel[FP_SABER_DEFENSE]];

		if ( self->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_3 )
		{
			parryTime = 50;
		}
					...
					{//close
						...
						{//be nice and stand still for him...
							...
							*/
							...
							TIMER_Set( NPC, "strafeRight", -1 );
							TIMER_Set( NPC, "noStrafe", Q_irand( 500, 1000 ) );
							TIMER_Set( NPC, "noturn", Q_irand( 250, 500 )*(3-G_DifficultyLimit()) );
...
void NPC_Jedi_Pain( gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod,int hitLoc )
{
	...
	{//back off
		...
		{//less for Desann
			self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + (3-G_DifficultyLimit())*50;
		}
		else if ( self->NPC->rank >= RANK_LT_JG )
		{
			self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + (3-G_DifficultyLimit())*100;//300
		}
		else
		{
			self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + (3-G_DifficultyLimit())*200;//500
		}
		...
		{//see if we should turn on absorb
			...
			{//know absorb and not already using it
				if ( other->s.number >= MAX_CLIENTS //enemy is an NPC
					|| Q_irand( 0, G_DifficultyLimit()+1 ) )//enemy is player
				{
				...
				{//on the ground
					...
					{//not going to try to jump
						...
						*/
						...
						if ( Q_irand( 0, G_DifficultyLimit()+1 ) //50% chance on easy, 66% on medium, 75% on hard
			...
			&& NPC->client->NPC_class != CLASS_ROCKETTROOPER )
		{
			...
			{//Tavion will kick in force speed if the player does...
					...
					&& !(NPC->client->ps.forcePowersActive & (1<<FP_SPEED)) )
				{
					...
					if ( !Q_irand( 0, chance ) )
					{
						if ( NPC->client->ps.forcePowerLevel[FP_SPEED] >= NPC->enemy->client->ps.forcePowerLevel[FP_SPEED] )
						{
							ForceSpeed( NPC );
						}
					}
...
qboolean Kothos_HealRosh( void )
{
		...
		&& NPC->client->leader->client )
	{
			...
			&& G_ClearLineOfSight( NPC->client->leader->client->renderInfo.eyePoint, NPC->client->renderInfo.eyePoint, NPC->s.number, MASK_OPAQUE ) )
		{
			...
			NPC->client->leader->health += Q_irand( 1+G_DifficultyLimit()*2, 4+G_DifficultyLimit()*3 );//from 1-5 to 4-10
			if ( NPC->client->leader->client )
			{
				...
				{//let him get up now
					...
					G_PlayEffect( 
						G_EffectIndex( "force/kothos_recharge.efx" ), 
						NPC->client->leader->playerModel, 
						0, 
						NPC->client->leader->s.number, 
						NPC->client->leader->currentOrigin, 
						NPC->client->leader->client->ps.torsoAnimTimer, 
						qfalse 
					);
				...
				*/
				...
				else if ( NPC->enemy && DistanceSquared( NPC->enemy->currentOrigin, NPC->currentOrigin ) < Twins_DangerDist() )
				{
					if ( NPC->enemy && Kothos_Retreat() )
					{
						...
						{
							if ( NPC->painDebounceTime > level.time
								|| (NPC->health < 100 && Q_irand(-20, (G_DifficultyLimit()+1)*10) > 0 )
								|| !Q_irand( 0, 80-(G_DifficultyLimit()*20) ) )
							{
								NPC->flags &= ~FL_LOCK_PLAYER_WEAPONS;
								switch ( Q_irand( 0, 7+G_DifficultyLimit() ) )//on easy: no lightning
								{
								case 0:
								case 1:
								case 2:
								case 3:
									...
									NPC->client->ps.weaponTime = Q_irand( 1000, 3000 )+(2-G_DifficultyLimit())*1000;
								...
								case 5:
									...
									NPC->client->ps.weaponTime = Q_irand( 3000, 6000 )+(2-G_DifficultyLimit())*2000;
