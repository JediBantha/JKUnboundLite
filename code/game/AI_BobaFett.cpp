...
void	Boba_DustFallNear(const vec3_t origin, int dustcount)
{
	...
	for (int i=0; i<dustcount; i++)
	{
		...
		gi.trace (&testTrace, origin, NULL, NULL, 
			testEndPos, (player && player->inuse)?(0):(ENTITYNUM_NONE), MASK_SHOT, (EG2_Collision)0, 0 );
...
qboolean Boba_Flying( gentity_t *self )
{
	assert( self 
		&& self->client 
		&& self->client->NPC_class == CLASS_BOBAFETT );//self->NPC &&
...
void Boba_FireFlameThrower( gentity_t *self )
{
	...
	int			min	= (1+G_DifficultyLimit()), max = (min + 4);
	int			damage		= Q_irand( min, max );
	int			dflags		= (DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC|DAMAGE_IGNORE_TEAM);
	...
	{
		G_Damage( traceEnt, self, self, dir, tr.endpos, 
			damage, dflags, MOD_FIRE, HL_NONE );
...
void Boba_DoFlameThrower( gentity_t *self )
{
	...
	if (!(NPCInfo->aiFlags&NPCAI_FLAMETHROW) 
		&& TIMER_Done(self, "nextAttackDelay"))
	{
...
void	Boba_Fire()
{
	...
	if (ucmd.buttons&BUTTON_ATTACK)
	{
		...
		case WP_BLASTER:
			...
			break;
		case WP_REPEATER:
			if (TIMER_Done(NPC, "nextRepeaterFireDecide"))
			{
 			 	if (Q_irand(0, (NPC->count*2)+3)>2)
				{
			 		TIMER_Set(NPC, "nextRepeaterFireDecide", Q_irand(3000, 8000));
					
					if ( (NPCInfo->scriptFlags & SCF_ALT_FIRE) )
					{
						Boba_Printf("ALT FIRE Off");
						NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
						NPC_ChangeWeapon(WP_BOWCASTER);
					}
				}
				else
				{
					TIMER_Set(NPC, "nextRepeaterFireDecide", Q_irand(2000, 5000));
			...
			break;
		case WP_BOWCASTER:
			if (TIMER_Done(NPC, "nextBowcasterFireDecide"))
			{
 			 	if (Q_irand(0, (NPC->count*2)+3)>2)
				{
			 		TIMER_Set(NPC, "nextBowcasterFireDecide", Q_irand(3000, 8000));
				...
				{
					TIMER_Set(NPC, "nextBowcasterFireDecide", Q_irand(2000, 5000));
...
void	Boba_TacticsSelect()
{
	...
	if (nextState!=NPCInfo->localState)
	{
		...
		case BTS_RIFLE:
			...
			{
			case 1:
				if ( (NPC->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_BLASTER_PISTOL )) != 0 )
				{
				...
				break;
			case 2:
				if ( (NPC->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_BOWCASTER )) != 0 )
				{
				...
				break;
			case 3:
				if ( (NPC->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_REPEATER )) != 0 )
				{
		...
		case BTS_MISSILE:
			...
			{
			case 1:
				if ( (NPC->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_DEMP2 )) != 0 )
				{
					Boba_ChangeWeapon(WP_DEMP2);
				}
				else
				{
			...
			case 2:
				...
				break;
			case 3:
				if ( (NPC->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_FLECHETTE )) != 0 )
				{
					Boba_ChangeWeapon(WP_FLECHETTE);
				}
				else
				{
					Boba_ChangeWeapon(WP_ROCKET_LAUNCHER);
				...
				break;
			case 4:
				if ( (NPC->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_CONCUSSION )) != 0 )
				{
