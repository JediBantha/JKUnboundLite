...
//---------------------------------------------------------
{
	...
	float		baseVel = BOWCASTER_VELOCITY;
	float		vel;
	...
	if ( ent->s.number != 0 )
	{
		...
		{
			baseVel = floor((float)BRYAR_PISTOL_VEL * (1.0f + (0.1f * (g_spskill->integer - 2))));

			if ( baseVel > Q3_INFINITE )
			{
				baseVel = Q3_INFINITE;
			}
		}
...
//	}
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		vel = baseVel * ( Q_flrand(-1.0f, 1.0f) * BOWCASTER_VEL_RANGE + 1.0f );
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( start, forwardVec, velocity, 10000, ent, qtrue );
