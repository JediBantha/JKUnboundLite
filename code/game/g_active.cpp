...
qboolean G_ControlledByPlayer( gentity_t *self )
{
	if ( self 
		&& self->NPC 
		&& self->NPC->controlledTime > level.time )
	{//being controlled
		gentity_t *controller = &g_entities[0];
		
		if ( controller->client 
			&& controller->client->ps.viewEntity == self->s.number )
...
qboolean G_ValidateLookEnemy( gentity_t *self, gentity_t *enemy )
{
	...
	else
	{
		...
		if ( enemy->health <= 0 
			&& ((level.time-enemy->s.time) > 3000 || 
			!InFront( enemy->currentOrigin, self->currentOrigin, self->client->ps.viewangles, 0.2f ) || 
			DistanceHorizontal( enemy->currentOrigin, self->currentOrigin ) > 16384 ) )//>128
	...
	if ( (!InFront( enemy->currentOrigin, self->currentOrigin, self->client->ps.viewangles, 0.0f) || 
		!G_ClearLOS( self, self->client->renderInfo.eyePoint, enemy ) )
		&& ( DistanceHorizontalSquared( enemy->currentOrigin, self->currentOrigin ) > 65536 || 
			fabs(enemy->currentOrigin[2]-self->currentOrigin[2]) > 384 )  )
...
void P_WorldEffects( gentity_t *ent ) 
{
	...
	// Poisoned?
	if ( (ent->client->poisonDamage) 
		&& (ent->client->poisonTime < level.time) )
	{
		int	min = (1 + G_DifficultyLimit()), max = (min + G_DifficultyLimit());
		int	damage = Q_irand( min, max );
		int	dflags = (DAMAGE_NO_KNOCKBACK|DAMAGE_NO_ARMOR);

		ent->client->poisonDamage -= 2;
		ent->client->poisonTime = level.time + 1000;

		G_Damage( ent, NULL, NULL, 0, 0, damage, dflags, MOD_POISON );
	...
	{ //we're in space, check for suffocating and for exiting
		...
		float	min = (maxHealth * 0.2f), max = (maxHealth * 0.4f);
		...
		{ //check for suffocation
			if ( !NPC_IsDroidClass( ent ) )
			{
				if (ent->client->inSpaceSuffocation < level.time)
				{ //suffocate!
					if (ent->health > 0)
					{ //if they're still alive..
						G_Damage(ent, spacetrigger, spacetrigger, NULL, ent->client->ps.origin, 
							Q_irand(min, max), DAMAGE_NO_ARMOR, MOD_SPACE);
	...
	if ( ent->s.powerups & ( 1 << PW_SHOCKED_RED ) )
	{
		if ( ent->client->redLightningDamageDebounce < level.time )
		{
			gentity_t	*attacker = ent->enemy;
			
			int			min = 1, max = (min + 2);
			int			damage = Q_irand( 1, 3 );
			int			dflags = (DAMAGE_NO_KNOCKBACK);
			int			minRate = 100, maxRate = (minRate * 2);

			if ( attacker && attacker->client )
			{
				int	lightningLevel = attacker->client->ps.forcePowerLevel[FP_LIGHTNING];
				int	modPowerLevel = -1;

				min = (lightningLevel - 5);
				...
				damage = Q_irand( min, max );
				minRate = (100 - (10 * (lightningLevel - 6)));
				...
				if ( ent->client )
				{
					modPowerLevel = WP_AbsorbConversion(ent, ent->client->ps.forcePowerLevel[FP_ABSORB],
						attacker, FP_LIGHTNING, lightningLevel, 0);
				}
				...
				if ( modPowerLevel != -1 )
				{
					...
					else
					{
						...
						case 2:
							...
							break;
						case 3:
							damage = floor((float)damage / 1.5f);
							break;
						default:
							break;
						}
				...
				if ( damage )
				{
					G_Damage( ent, attacker, attacker, 0, 0, damage, dflags, MOD_ELECTROCUTE );
				}
			}
			else
			{
				G_Damage( ent, NULL, NULL, 0, 0, Q_irand( 1, 3 ), dflags, MOD_ELECTROCUTE );
			}
			
			ent->client->redLightningDamageDebounce = level.time + Q_irand( minRate, maxRate );
		}
	}
}
...
//				}
 	...
 	if ( !self->client || 
		self->client->ps.lastOnGround + 300 < level.time || 
		( self->client->ps.lastOnGround + 100 < level.time ) )
	{
		vec3_t dir1, dir2;
		float force = 0, dot;
		...
		if ( other->material == MAT_GLASS || 
			...
			((other->svFlags&SVF_BBRUSH) 
				&& (other->spawnflags & 8/*THIN*/)) )//(other->absmax[0]-other->absmin[0]<=32||other->absmax[1]-other->absmin[1]<=32||other->absmax[2]-other->absmin[2]<=32)) )
		...
		if ( pSelfVeh && pSelfVeh->m_pVehicleInfo->type != VH_ANIMAL )
		{
 			if ( (magnitude >= 80) && (self->painDebounceTime < level.time) )
			{
				...
				if (!other || !other->client)
				{
					...
					//------------------------------------------------------------
					if ((pSelfVeh->m_ulFlags&VEH_OUTOFCONTROL) 
						&& (vehHitPercent>0.5f || trace->plane.normal[2]<0.5f || velocity[2]<-50.0f))
					{
						vehDFlags |= DAMAGE_IMPACT_DIE;
					}
					...
					//-----------------------------------------------------
					if (vehHitPercent>0.9f 
						&& (level.time - self->lastImpact)>2000 
						&& vehSpeed>300.0f)
					{
	...
	*/
				...
				{//if in a vehicle when land on someone, always knockdown, throw, damage
					...
					{//didn't hit owner
						...
						G_Damage( other, self, self, velocity, self->currentOrigin, 
							force, DAMAGE_NO_ARMOR|DAMAGE_EXTRA_KNOCKBACK, MOD_IMPACT );
					}
				}
				...
				else if ( other->takedamage )
				{
					...
					{//aw, fuck it, clients no longer take impact damage from other clients, unless you're the player
						...
						{
							G_Damage( other, self, self, velocity, self->currentOrigin, 
								floor(force), DAMAGE_NO_ARMOR, MOD_IMPACT );
						}
					}
					else
					{
			...
			&& !(self->flags&FL_NO_IMPACT_DMG))
		{
				...
				|| ( magnitude >= 700 ) )//health here is used to simulate structural integrity
			{
				if ( (self->s.weapon == WP_SABER || 
					self->s.number<MAX_CLIENTS || 
					(self->client && (self->client->NPC_class==CLASS_BOBAFETT || self->client->NPC_class==CLASS_ROCKETTROOPER))) 
					&& self->client 
	...
	*/
					...
					G_Damage( self, NULL, NULL, NULL, self->currentOrigin, 
						magnitude/2, DAMAGE_NO_ARMOR, MOD_FALLING );//FIXME: MOD_IMPACT
				}
			}
		}
...
void G_MatchPlayerWeapon( gentity_t *ent )
{
	...
	{//player is around
		...
		if ( g_entities[0].client->ps.weapon > WP_DET_PACK )//WP_CONCUSSION
		{
...
{
	gclient_t	*client;
	int			residualTime = 1000;

	if ( g_spskill->integer > 1 )
	{
		residualTime /= g_spskill->integer;
	...
	while ( client->timeResidual >= residualTime )
	{
		client->timeResidual -= residualTime;
...
gentity_t *G_KickTrace( gentity_t *ent, vec3_t kickDir, float kickDist, vec3_t kickEnd, int kickDamage, float kickPush, qboolean doSoundOnWalls )
{
	...
	gi.trace( &trace, traceOrg, kickMins, kickMaxs, 
		traceEnd, ent->s.number, MASK_SHOT, (EG2_Collision)0, 0 );//clipmask ok?
		...
		{//we hit an entity
			if ( hitEnt->client )
			{
				...
				{//FIXME: this should not always work
					...
					{//hurt it
						G_Damage( hitEnt, ent, ent, kickDir, trace.endpos, 
							kickDamage, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_KILL, MOD_MELEE );
					}
...
qboolean G_CheckRollSafety( gentity_t *self, int anim, float testDist )
{
	...
	gi.trace( &trace, self->currentOrigin, self->mins, self->maxs, 
		testPos, self->s.number, contents, (EG2_Collision)0, 0 );
...
void G_FixMins( gentity_t *ent )
{
	...
	gi.trace( &trace, ent->currentOrigin, ent->mins, ent->maxs, 
		end, ent->s.number, ent->clipmask, (EG2_Collision)0, 0 );
	...
	{//can't turn during Kyle's grappling
		...
		{//actually do some damage during sequence
			...
			if ( TIMER_Done( ent, "grappleDamageDebounce" ) )
			{
				...
				if ( damage )
				{
					G_Damage( ent, ent->enemy, ent->enemy, NULL, ent->currentOrigin, 
						damage, dflags, MOD_MELEE );//MOD_IMPACT?
				}
			}
		}
...
void	ClientAlterSpeed(gentity_t *ent, usercmd_t *ucmd, qboolean	controlledByPlayer, float vehicleFrameTimeModifier)
{
	...
	if ( !pVeh )
	{
		...
		if ( client->ps.forcePowersActive & ( 1 << FP_SPEED ) )
		{
			int		speedLevel = client->ps.forcePowerLevel[FP_SPEED];
			...
			case 5:
				...
				break;
			default:
				speedBoost = (1.4 + (0.1 * (speedLevel - 5)));
				break;
			}

			client->ps.speed *= speedBoost;
		}
...
extern cvar_t	*g_skippingcin;
		...
		else
		{
			...
			if ( ent->client->ps.pm_type == PM_DEAD 
				&& cg.missionStatusDeadTime < level.time )
...
//		}
	...
	else
	{
		...
		if ( player 
			&& player->client 
			&& player->client->ps.viewEntity == ent->s.number )
		{
		...
		{//no jumping in atst
			...
			{//ATST crushes anything underneath it
					...
					&& under->takedamage )
				{
					...
					G_Damage( under, ent, ent, down, under->currentOrigin, 
						100, 0, MOD_CRUSH );
				}
