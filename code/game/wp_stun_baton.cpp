...
void WP_FireStunBaton( gentity_t *ent, qboolean alt_fire )
{
	...
	gi.trace ( &tr, start, mins, maxs, 
		end, ent->s.number, CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_SHOTCLIP, (EG2_Collision)0, 0 );
	...
	{
		int	damage = STUN_BATON_DAMAGE;

		if ( ent->client->ps.forcePowersKnown & ( 1 << FP_LIGHTNING ) )
		{
			int		lightningLevel = ent->client->ps.forcePowerLevel[FP_LIGHTNING];
			float	lightningMult = (0.15f + (0.15f * (lightningLevel)));

			damage += floor((float)damage * lightningMult);
		}
...
//		G_Sound( tr_ent, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
	...
	else if ( tr_ent->svFlags & SVF_GLASS_BRUSH || 
		( tr_ent->svFlags & SVF_BBRUSH && tr_ent->material == 12 )) // material grate...we are breaking a grate!
	{
