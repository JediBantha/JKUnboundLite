...
//---------------------------------------------------------
{
	...
	int			rageLevel = ent->client->ps.forcePowerLevel[FP_RAGE];
	int			damage = ent->s.number ? (G_DifficultyLimit()*2)+1 : 3;
	...
	gi.trace ( &tr, muzzle, mins, maxs, 
		end, ent->s.number, MASK_SHOT, (EG2_Collision)0, 0 );
	...
	if ( tr_ent && tr_ent->takedamage )
	{
		...
		{
			float	rageMult = (0.25f + (0.25f * (rageLevel - 1)));

			damage += floor((float)damage * rageMult);
		}
