...
void ClientUserinfoChanged( int clientNum ) 
{
	...
	// set max health
	maxHealth = 999;//100;
	health = Com_Clampi( 1, 999, atoi( Info_ValueForKey( userinfo, "handicap" ) ) );
...
static void Player_RestoreFromPrevLevel(gentity_t *ent, SavedGameJustLoaded_e eSavedGameJustLoaded)
{
	...
	if (client)	// though I can't see it not being true...
	{
		...
		{//				|general info				  |-force powers |-saber 1										   |-saber 2										  |-general saber
			...
			sscanf( s, "%i %i %i %i %i %i %i %i %i %f %f %f %i %i %i %i %i %s %i %i %i %i %i %i %i %i %u %u %u %u %u %u %u %u %s %i %i %i %i %i %i %i %i %u %u %u %u %u %u %u %u %i %i %i %i",
...
qboolean ClientSpawn(gentity_t *ent, SavedGameJustLoaded_e eSavedGameJustLoaded )
{
	...
	else
	{
		...
		for ( int item = INV_ELECTROBINOCULARS; item < INV_MAX; item++ )
		{
			switch ( item )
			{
			case INV_ELECTROBINOCULARS:
			case INV_BACTA_CANISTER:
				client->ps.inventory[item] = 1;
				break;
			}
...
//			}
		...
		{//player starts with absolutely no weapons
			...
			for ( int item = INV_ELECTROBINOCULARS; item < INV_MAX; item++ )
			{
				ent->client->ps.inventory[item] = 0;
			}
		}
