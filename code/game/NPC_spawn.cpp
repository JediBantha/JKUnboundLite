...
void NPC_SetMiscDefaultData( gentity_t *ent )
{
	...
	if ( ent->client->ps.weapon == WP_MELEE )
	{
		...
		{
			if ( ent->client->ps.forcePowersKnown & ( 1 << forcePower ) )
			{
				switch ( forcePower )
				{
				case FP_PUSH:
	...
	if ( Q_stricmp("tavion_scepter", ent->NPC_type ) == 0
	...
	case TEAM_NEUTRAL:
		...
		if ( Q_stricmp( ent->NPC_type, "gonk" ) == 0 )
		{
			...
			{
				ent->client->ps.batteryCharge = MAX_BATTERIES * 0.95f;
			}
			else
			{
				...
				case 0:	//	EASY
					...
					break;
				case 1:	//	MEDIUM
					ent->client->ps.batteryCharge = MAX_BATTERIES * 0.65f;
					break;
				case 2:	//	HARD
				default:
					{
						float	diffMult = (0.5f - (0.15f * (g_spskill->integer - 2)));
						...
						ent->client->ps.batteryCharge = MAX_BATTERIES * diffMult;
					}
					break;
				}
			}
	...
	case TEAM_ENEMY:
		{
			...
			{
			case CLASS_SHADOWTROOPER:
				if ( !Q_stricmpn( "shadowtrooper", ent->NPC_type, 13 ) )
				{//FIXME: a spawnflag?
					Jedi_Cloak( ent );
				}
...
void NPC_Begin (gentity_t *ent)
{
	...
	else if ( ent->NPC->stats.health )	// Was health supplied in NPC.cfg?
	{
		...
		{// up everyone except jedi
			...
			{//bosses are a bit different
				ent->NPC->stats.health = ceil((float)ent->NPC->stats.health*0.75f + ((float)ent->NPC->stats.health/4.0f*G_DifficultyLimit())); // 75% on easy, 100% on medium, 125% on hard
			}
			else
			{
				ent->NPC->stats.health += ent->NPC->stats.health/4 * G_DifficultyLimit(); // 100% on easy, 125% on medium, 150% on hard
			}
		}
	...
	{//tweak yawspeed for these NPCs based on difficulty
		if ( g_spskill->integer < 0 )
		{
			ent->NPC->stats.yawSpeed *= 0.5f;

			if ( ent->client->NPC_class == CLASS_IMPWORKER )
			{
				ent->NPC->stats.aim -= Q_irand( 4, 8 );
			}
		}
		else
		{
