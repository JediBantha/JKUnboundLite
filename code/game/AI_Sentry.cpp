...
void Sentry_Fire (void)
{
	...
	int			bolt;
	int			damage = REPEATER_NPC_DAMAGE_HARD;//5;
	float		velocity = 1600;
...
//	G_Sound( NPC, G_SoundIndex("sound/chars/sentry/misc/shoot.wav"));
	...
	if ( g_spskill->integer <= 0 )
	{
		...
		missile->damage = REPEATER_NPC_DAMAGE_EASY;
	}
	else if ( g_spskill->integer == 1 )
	{
		NPC->attackDebounceTime += 100;
		missile->damage = REPEATER_NPC_DAMAGE_NORMAL;
	}
...
void Sentry_Hunt( qboolean visible, qboolean advance )
{
	...
	speed = SENTRY_FORWARD_BASE_SPEED + SENTRY_FORWARD_MULTIPLIER * G_DifficultyLimit();
