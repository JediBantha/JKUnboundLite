...
void shield_power_converter_use( gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if ( self->setTime < level.time )
	{
		...
		float	maxHealth = (activator->client->ps.stats[STAT_MAX_HEALTH] * 2);//100;
		int		giveAmount = MAX_AMMO_GIVE;
		...
		dif = maxHealth - activator->client->ps.stats[STAT_ARMOR]; // FIXME: define for max armor?
		...
		else if ( shieldConDone )//( activator->client->ps.stats[STAT_ARMOR] >= maxHealth ) // FIXME: define for max
		{
			// play full sound
