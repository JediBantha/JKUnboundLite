			...
			{//have one in left
				...
				{//okay to drop it
					...
					{//have a valid string to use for saberType
						...
						if ( G_DropSaberItem( 
							self->client->ps.saber[1].name, 
							self->client->ps.saber[1].blade[0].color, 
							self->client->renderInfo.handLPoint, 
							self->client->ps.velocity, 
							self->currentAngles ) != NULL )
						{//dropped it
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath, int dflags, int hitLoc )
{
	...
	if ( self->client->ps.pm_type == PM_DEAD 
		&& (meansOfDeath != MOD_SNIPER || (self->flags & FL_DISINTEGRATED)) )
...
//		G_SetMissionStatusText( attacker, meansOfDeath );
	...
	*/
	...
	{//imp with a key on his arm
		...
		(self->client->ps.legsAnim == BOTH_FALLDEATH1INAIR && self->client->ps.torsoAnim == BOTH_FALLDEATH1INAIR) || 
		(self->client->ps.legsAnim == BOTH_FALLDEATH1 && self->client->ps.torsoAnim == BOTH_FALLDEATH1) )
	{
	...
	{// normal death
		...
		if ( anim == -1 )
		{
			...
			{//electrocuted
				...
				meansOfDeath == MOD_FORCE_DRAIN || 
				meansOfDeath == MOD_POISON || 
				meansOfDeath == MOD_SPACE )
			{//drowned
				anim = BOTH_DEATH17;
			}
		...
		if ( anim == -1 )
		{
			...
			case MOD_FORCE_GRIP:
				...
				break;
			case MOD_CRUSH:
				if ( self->s.eFlags & EF_FORCE_GRIPPED )
				{
					anim = BOTH_DEATH17;
				}
				break;
			case MOD_FORCE_DRAIN:
				//if ( self->s.eFlags & EF_FORCE_DRAINED )
				{
					anim = BOTH_DEATH17;
				}
				break;
	...
	{//we did dismemberment and our death anim is okay to override
		if ( hitLoc == HL_HAND_RT 
			&& self->locationDamage[hitLoc] >= Q3_INFINITE 
			&& cliff_fall != 2 
			&& self->client->ps.groundEntityNum != ENTITYNUM_NONE )
...
void PlayerPain( gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc )
{
	...
	else
	{
		...
		{//first time hit this frame and not in godmode
			...
			if ( !Q3_TaskIDPending( self, TID_CHAN_VOICE ) )
			{
				...
				{//took damage myself, not just armor
					...
					{
					case MOD_POISON:
					case MOD_SPACE://this is covered in g_active.cpp
						break;
					case MOD_GAS:
					...
					case MOD_FORCE_DRAIN:
						...
						{
							if ( self->client->ps.eFlags & EF_FORCE_DRAINED )
							{
								if ( TIMER_Done( self, "chokeSound" ) )
								{
									TIMER_Set( self, "chokeSound", Q_irand( 1000, 2000 ) );
									G_SpeechEvent( self, Q_irand( EV_CHOKE1, EV_CHOKE3 ) );
								}
							}
						}
		...
		if ( damage != -1 
			&& (mod==MOD_MELEE || damage==0/*fake damage*/ || (Q_irand( 0, 10 ) <= damage 
				&& self->client->damage_blood)) )
		{//-1 == don't play pain anim
			if ( ( ((mod==MOD_SABER || mod==MOD_MELEE) 
				&& self->client->damage_blood) || mod == MOD_CRUSH ) 
				&& (self->s.weapon == WP_SABER || self->s.weapon == WP_MELEE || cg.renderingThirdPerson) )//FIXME: not only if using saber, but if in third person at all?  But then 1st/third person functionality is different...
			{//FIXME: only strong-level saber attacks should make me play pain anim?
				...
				{//strong attacks and spins cannot be interrupted by pain, no pain when in knockdown
					...
					if ( (parts == SETANIM_BOTH && damage > 30) || 
						(self->painDebounceTime > level.time && damage > 10) )
...
int CheckArmor (gentity_t *ent, int damage, int dflags, int mod)
{
		...
		case MOD_SABER:
			if (!(ent->flags&FL_SHIELDED))
			{
				...
				if (mod == MOD_SABER)
				{
					...
					//----------------------------------------------------
					return (int)((float)(damage)*0.5f);//was 0.75f
				}
	...
	else
	{
		...
		int	armorThreshold = (client->ps.stats[STAT_MAX_HEALTH] / 2);
		...
		if ( count > armorThreshold ) // MAX_HEALTH is considered max armor. Or so I'm told.
		{
			save = damage;
		}
		else
		{
			...
			else
			{
				...
				if ( !ent->s.number && g_spskill->integer > 2 )
				{
					...
					if ( armorProtection < 0.01 )
					{
						armorProtection = 0.01;
					}
					else if ( armorProtection > 1.0 )
					{
						armorProtection = 1.0;
					}
				}
...
void G_CheckKnockdown( gentity_t *targ, gentity_t *attacker, vec3_t newDir, int dflags, int mod )
{
	...
	if ( !targ->client || 
		targ->client->NPC_class == CLASS_PROTOCOL || 
		!G_StandardHumanoid( targ ) )
	{
		return;
	}
	...
	if ( targ->client->ps.velocity[2] > 100 
		&& strength > Q_irand( 150, 350 ) )//600 ) )
...
static int G_CheckForLedge( gentity_t *self, vec3_t fallCheckDir, float checkDist )
{
	...
	gi.trace( &tr, self->currentOrigin, self->mins, self->maxs, 
		end, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
	...
	gi.trace( &tr, start, self->mins, self->maxs, 
		end, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
...
qboolean G_ImmuneToGas( gentity_t *ent )
{
	...
	{
	case CLASS_VEHICLE:
		switch ( ent->m_pVehicle->m_pVehicleInfo->type )
		{
		case VH_ANIMAL:
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, const vec3_t dir, const vec3_t point, int damage, int dflags, int mod, int hitLoc )
{
	...
	if ( (targ->flags&FL_DMG_BY_SABER_ONLY) 
		&& mod != MOD_SABER )
	...
	{//sabers do less damage to mark1's and atst's, and to hazard troopers and assassin droids
		if ( targ->client )
		{
			...
			case CLASS_MARK1:
				...
				{
					maxDamage = ceil((float)targ->max_health / (4.0f + g_spskill->value));
				}
				break;
			case CLASS_ASSASSIN_DROID://takes 2 hits to kill on easy, 3 on medium, 4 on hard
				maxDamage = ceil((float)targ->max_health / (2.0f + g_spskill->value));
				break;
			case CLASS_HAZARD_TROOPER://takes 3 hits to kill on easy, 4 on medium, 5 on hard
				maxDamage = ceil((float)targ->max_health / (3.0f + g_spskill->value));
				break;
			case CLASS_SHADOWTROOPER:
				if ( !Q_stricmp( "ShadowTrooper", targ->NPC_type ) || 
					!Q_stricmp( "ShadowTrooper2", targ->NPC_type ) )
				{
					maxDamage = ceil((float)targ->max_health / (2.0f + g_spskill->value));
				}
				break;
	...
	*/
	...
	{
		float	targValue = 1.0f, attackerValue = 1.0f;
		...
		if ( !attacker->s.number )
		{
			if ( targ->s.number != 0 )
			{
				...
				{
					targValue = (1.0f - (0.25f * (g_spskill->integer - 2)));
					...
					damage = ceil((float)damage * targValue);
				...
				if ( !(OnSameTeam( attacker, targ )) )
				{
					if ( g_spskill->integer > 2 )
					{
						...
						case MOD_SPACE:
							break;
						default:
							attackerValue = (1.0f + (0.5f * (g_spskill->integer - 2)));
							damage = ceil((float)damage * attackerValue);
							break;
						}
					}
				}
	...
	*/
	...
	{//force protections
		...
		{
			int		rageLevel = targ->client->ps.forcePowerLevel[FP_RAGE];
			float	absorbed = 0;

			switch ( rageLevel )
			{
			case 1:
				absorbed = ceil((float)damage * 0.5f);
				break;
			case 2:
				absorbed = ceil((float)damage * 0.75f);
				break;
			case 3:
			default:
				absorbed = ceil((float)damage * 0.9f);
				break;
			}
			...
			float	levelMult = (0.05f + (0.05f * (rageLevel - 1)));
			...
			if ( damage >= 2 )
			{
				targ->client->ps.forcePower += floor((float)absorbed * levelMult);

				if ( targ->client->ps.forcePower > targ->client->ps.forcePowerMax )
				{
					targ->client->ps.forcePower = targ->client->ps.forcePowerMax;
				}
			}
		...
		{
			int	protectLevel = targ->client->ps.forcePowerLevel[FP_PROTECT];
			int	absorbed = 0;
			...
			else
			{
				...
				case MOD_MELEE:
					...
					default:
						if ( !targ->s.number )
						{
							float	levelMult = (0.05f - (0.01f * (protectLevel - 6)));
							...
							else
							{
								...
								else
								{
									...
									case MOD_ELECTROCUTE:
									case MOD_FIRE:
										if ( targ->client->ps.forcePowerDamageDebounce[FP_PROTECT] < level.time )
										{
											int	levelDebounce = (50 + (50 * (protectLevel - 4)));

											if ( levelDebounce > Q3_INFINITE )
											{
												levelDebounce = Q3_INFINITE;
											}
											
											targ->client->ps.forcePower -= ceil((float)absorbed * levelMult);
											targ->client->ps.forcePowerDamageDebounce[FP_PROTECT] = level.time + levelDebounce;
										}
										break;
									default:
										targ->client->ps.forcePower -= ceil((float)absorbed * levelMult);
						...
						else
						{
							...
							{
								float	levelMult = (0.05f - (0.01f * (protectLevel - 5)));
								...
								damage = ceil((float)damage * levelMult);
							}
						}
						break;
					case FORCE_LEVEL_3://one-tenth damage
					case FORCE_LEVEL_2://half damage
					case FORCE_LEVEL_1://three-quarters damage
					case FORCE_LEVEL_0:
						if ( damage <= 1 )
						{
							damage = 0;
						...
						{
							float	levelMult = (0.75f - (0.25f * (protectLevel - FORCE_LEVEL_1)));

							if ( levelMult < 0.1f )
							{
								levelMult = 0.1f;
							}
							else if ( levelMult > 0.75f )
							{
								levelMult = 0.75f;
							}

							damage = ceil((float)damage*0.75f);//was 0.1f);
						}
						break;
					}
					break;
		...
		{
			int		absorbLevel = targ->client->ps.forcePowerLevel[FP_ABSORB];
			int		forceSpent = 1;
			float	levelMult = (1.0f + (0.25f * (absorbLevel - FORCE_LEVEL_2)));
			float	maxForce = floor((float)targ->client->ps.forcePowerMax * levelMult);
			...
			*/
			...
			case MOD_SCEPTER_ALT:
				if ( targ->client )
				{
					WP_AbsorbConversion( targ, absorbLevel, attacker, FP_LIGHTNING, FORCE_LEVEL_3, 0 );
				}
				...
				if ( modPowerLevel != -1 )
				{
					...
					else
					{
						...
						{
						case 1:
							absorbed = floor((float)damage / 1.5f);
							break;
						case 2:
							absorbed = floor((float)damage / 2.0f);
							break;
						case 3:
							absorbed = floor((float)damage / 4.0f);
							break;
						default:
							break;
						}
					...
					if ( !(targ->client->ps.forcePowersActive & ( 1 << FP_PROTECT ) ) )
					{
						targ->client->ps.forcePower += ((absorbed/3) * absorbLevel);

						if ( targ->client->ps.forcePower > maxForce )
						{
							targ->client->ps.forcePower = maxForce;
						}
					}
			...
			case MOD_ENERGY:
			case MOD_ENERGY_SPLASH:
				if ( ((targ->client->ps.forcePowersKnown & (1 << FP_PROTECT)) != 0 
					&& targ->client->ps.forcePowerLevel[FP_PROTECT] > FORCE_LEVEL_2)
					&& absorbLevel > FORCE_LEVEL_2 )
				{
					int		protectLevel = targ->client->ps.forcePowerLevel[FP_PROTECT];
					float	protectMult = (0.15f + (0.15f * (protectLevel - 3)));
					...
					float	absorbed = ceil((float)damage * protectMult);
	...
	else if ( mod == MOD_CRUSH || mod == MOD_FORCE_GRIP )
	{
		knockback = 0;
	}
		...
		case MOD_LAVA:
			...
			if (mod==MOD_SLIME)
			{
				...
				//for (int i=0; i<numPuffs; i++)
				{
					...
					gi.trace (&testTrace, testStartPos, NULL, NULL, 
						testEndPos, ENTITYNUM_NONE, MASK_SHOT, G2_COLLIDE, 0);
...
//					CG_DrawEdge(testStartPos,	testEndPos,	EDGE_IMPACT_POSSIBLE);
		...
		if ( targ->client->NPC_class == CLASS_VEHICLE )
		{
				...
				)
			{
				...
				// DISMEMBER THE FRONT PART OF THE MODEL
				{
					...
					gi.trace( &trace, limb->s.pos.trBase, limb->mins, limb->maxs, 
						limb->s.pos.trBase, limb->s.number, limb->clipmask, (EG2_Collision)0, 0 );
	...
	{
		gi.Printf( 
			"[%d]client:%i health:%i damage:%i armor:%i hitloc:%s\n", 
			level.time, 
			targ->s.number, 
			targ->health, 
			take, 
	...
	if ( take || (dflags&DAMAGE_NO_DAMAGE) )
	{
		...
		{//two clients
			...
			if ( player->client->ps.viewEntity 
				&& targ->s.number == player->client->ps.viewEntity )
			{
		...
		//	}
			...
			if ( player->client->ps.viewEntity 
				&& attacker->s.number == player->client->ps.viewEntity )
			{
		...
		//	}
			...
			{//on opposite team
				...
				if (targUndying)
				{
					...
					{
						if ( targ->NPC == NULL || 
							!(targ->NPC->aiFlags&NPCAI_ROSH) || 
							!Rosh_TwinPresent( targ ) )
		...
		if ( targ->s.number == 0 )
		{
			...
			&& (!targ->NPC || !targ->NPC->timeOfDeath) 
			&& (mod == MOD_SABER || attacker->s.weapon != WP_SABER || !attacker->enemy || attacker->enemy->s.weapon != WP_SABER) )//keep enemy jedi over shooters
		...
		if ( targ->health > 0 
			&& targ->NPC 
			&& targ->NPC->surrenderTime > level.time )
...
qboolean CanDamage (gentity_t *targ, const vec3_t origin) 
{
	...
	*/
	gi.trace ( &tr, origin, vec3_origin, vec3_origin, 
		dest, ENTITYNUM_NONE, MASK_SOLID, (EG2_Collision)0, 0);
