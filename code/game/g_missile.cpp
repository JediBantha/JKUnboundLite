...
void G_ReflectMissile( gentity_t *ent, gentity_t *missile, vec3_t forward )
{
	...
	if ( ent 
		...
		&& (owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_2 || 
			(owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE]>FORCE_LEVEL_1 
				&& !Q_irand( 0, 3 ))) )
...
void NoghriGasCloudThink( gentity_t *self )
{
	...
	{
		if ( !Q_irand( 0, 3-G_DifficultyLimit() ) )
		{
...
void G_MissileImpacted( gentity_t *ent, gentity_t *other, vec3_t impactPos, vec3_t normal, int hitLoc=HL_NONE )
{
	...
	if ( other->takedamage )
	{
		...
		if ( ent->damage )
		{
			...
			if( other->client )
			{
				...
				case CLASS_MARK1:
				case CLASS_MARK2:
				//case CLASS_PROTOCOL://no protocol, looks odd
			...
			{//a hit with demp2 decloaks saboteurs
				if ( other 
					&& other->client 
					&& other->client->NPC_class == CLASS_SABOTEUR )
	...
	if ( (other->takedamage && other->client ) || 
		(ent->s.weapon == WP_FLECHETTE && other->contents&CONTENTS_LIGHTSABER) )
	{
