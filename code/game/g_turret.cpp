...
qboolean place_stationary_shield( gentity_t *self, vec3_t origin, vec3_t angs )
{
	vec3_t		fwd, pos;
	vec3_t		mins, maxs;
	trace_t		tr;
		...
		&& tr.entityNum >= ENTITYNUM_WORLD )
	{
		...
		if ( shield )
		{
			...
			shield->contents |= (CONTENTS_SOLID|CONTENTS_OPAQUE);

			shield->e_UseFunc = useF_NULL;

			// we don't hurt us or anyone who belongs to the same team as us.
			if ( self->client )
			{
				shield->noDamageTeam = self->client->playerTeam;
			}

			G_Sound( self, G_SoundIndex( "sound/player/use_sentry" ));
			shield->activator = self;
