...
//		G_AddEvent( self, EV_GENERAL_SOUND, G_SoundIndex( "sound/weapons/change.wav" ));
	...
	case INV_CLOAK:
		...
		return;
		break;
	default:// else Bacta, seeker, sentry
...
static void Q3_SetForcePowerLevel ( int entID, int forcePower, int forceLevel )
{
	...
	if ( forceLevel < 0 )
	{
		...
		{
			Quake3Game()->DebugPrint( IGameInterface::WL_ERROR, "Q3_SetForcePowerLevel: Force power setting %d out of range (0-Infinite)\n", forceLevel );
			return;
		}
	}
	...
	if ( forceLevel < 0 )
	{
		self->client->ps.forcePowerLevel[forcePower]++;
