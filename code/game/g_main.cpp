...
int	G_DifficultyLimit( void )
{
	int	diffInt = g_spskill->integer;
...
static void G_DynamicMusicUpdate( void )
{
		...
		{//mad
			...
			{//alive
				...
				{//a turret that shot at me less than ten seconds ago
					...
					&& ent->client->ps.lastShotTime 
					&& ent->client->ps.lastShotTime - level.time < 10000 )
	...
	{//no active enemies, but look for missiles, shot impacts, etc...
		...
		{//FIXME: maybe tripwires and other FIXED things need their own sound, some kind of danger/caution theme
			...
			else if ( level.alertEvents[alert].owner 
				&& (level.alertEvents[alert].owner == player->enemy || 
					(level.alertEvents[alert].owner->client 
						&& level.alertEvents[alert].owner->client->playerTeam == player->client->enemyTeam) ) )
...
{//special-case alerts that would be a pain in the ass to have the ent's think funcs generate
	...
	if ( ent->contents == CONTENTS_LIGHTSABER 
...
//			continue;
		...
		if ( i == 0 )
		{
			...
			{ //hacking checks
				...
				{
					NPC_SetAnim( ent, NULL, SETANIM_TORSO, BOTH_CONSOLE1, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
				}
				else
				{
					ent->client->ps.torsoAnimTimer = 500;
				}
				...
				else if (!G_PointInBounds( ent->client->ps.origin, hacked->absmin, hacked->absmax ))
...
#define JETPACK_DEFUEL_RATE		(100 - (25 * G_DifficultyLimit())) //approx. 20 seconds of idle use from a fully charged fuel amt	//200
#define JETPACK_REFUEL_RATE		(JETPACK_DEFUEL_RATE * 2) //seems fair
			if ( JET_Flying( ent ) )
			{ //using jetpack, drain fuel
				if ( !ent->s.number )
				{
					...
					{
						if (ent->client->usercmd.upmove > 0)
						...
						else if ( (ent->client->usercmd.buttons & BUTTON_USE) )
						{
						...
						if ( ucmd.upmove < 0 )
						{
							ent->client->jetPackDebReduce = level.time + (JETPACK_DEFUEL_RATE + 25);
						}
						else
						{
...
#define CLOAK_DEFUEL_RATE		(100 - (25 * G_DifficultyLimit())) //approx. 20 seconds of idle use from a fully charged fuel amt	//200
#define CLOAK_REFUEL_RATE		(CLOAK_DEFUEL_RATE * 2) //seems fair	//150
...
if ( g_delayedShutdown->integer 
	&& delayedShutDown != 0 
	&& delayedShutDown < level.time )
	{
		assert(0);
