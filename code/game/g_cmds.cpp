...
extern void ItemUse_Cloak( gentity_t *ent );
extern void ItemUse_ForceGem(gentity_t *ent);
...
void G_Give( gentity_t *ent, const char *name, const char *args, int argc )
{
	...
	{
		ent->client->ps.stats[STAT_WEAPONS] = (1 << (WP_ATST_MAIN)) - ( 1 << WP_NONE );
		if ( !give_all )
			return;
	}
...
void Cmd_Undying_f (gentity_t *ent)
{
	...
	else
	{
		...
		if ( ent->client )
		{
			...
			else if ( max > Q3_INFINITE )
			{
				max = Q3_INFINITE;
			}

			ent->client->ps.stats[STAT_HEALTH] = ent->client->ps.stats[STAT_MAX_HEALTH] = max;//999
		}
	}
...
void Cmd_UseSeeker_f( gentity_t *ent )
{
	...
	if ( ent->client 
		...
		&& level.time > ent->client->ps.powerups[PW_SEEKER] )
	{
...
void Cmd_UseCloak_f(gentity_t *ent)
{
	...
	if ( ent->client->ps.inventory[INV_CLOAK] <= 0 )
	{
		return;
	}
	...
	if ( ent->client && ent->client->ps.inventory[INV_CLOAK] > 0 )
	{
		ItemUse_Cloak( ent );
	}
}

void Cmd_UseForceGem_f(gentity_t *ent)
{
	if ( ent->health < 1 || in_camera )
	{
		return;
...
void Cmd_UseInventory_f(gentity_t *ent)
{
	switch (cg.inventorySelect)
	{
		...
		case INV_SENTRY :
			...
			return;
		case INV_JETPACK:
			//This is covered by the JET_ scripts
			break;
		case INV_CLOAK:
			Cmd_UseCloak_f(ent);
			break;
		case INV_FORCE_GEM:
			Cmd_UseForceGem_f(ent);
			break;
		default :
			return;
...
//		G_Taunt( ent );
	...
	*/
	else if (Q_stricmp (cmd, "NPCdrive") == 0)
	{
		...
		Cmd_UseCloak_f( ent );
	else if (Q_stricmp (cmd, "use_force_gem") == 0)
		Cmd_UseForceGem_f( ent );
