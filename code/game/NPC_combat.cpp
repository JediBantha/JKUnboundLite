...
void G_AttackDelay( gentity_t *self, gentity_t *enemy )
{
	...
	{//delay their attack based on how far away they're facing from enemy
		...
		attDelay = (4-G_DifficultyLimit())*500;//initial: from 1000ms delay on hard to 2000ms delay on easy
		...
		*/
		...
		if ( attDelay > 4000+((2-G_DifficultyLimit())*3000) )
		{
			attDelay = 4000+((2-G_DifficultyLimit())*3000);
		}
...
//	}
	...
	if ( self->enemy == NULL )
	{
		...
		{//Hmm, how about sniper and bowcaster?
			...
			if ( self->client->playerTeam == TEAM_PLAYER )
			{
				G_AimSet( self, Q_irand( self->NPC->stats.aim - (5*(G_DifficultyLimit())), self->NPC->stats.aim - G_DifficultyLimit() ) );
			}
			else
			{
				int minErr = 3;
				...
				G_AimSet( self, Q_irand( self->NPC->stats.aim - (maxErr*(3-G_DifficultyLimit())), self->NPC->stats.aim - (minErr*(3-G_DifficultyLimit())) ) );
			}
		}
...
void NPC_AimAdjust( int change )
{
	...
	{
		int debounce = 500+(3-G_DifficultyLimit())*100;
