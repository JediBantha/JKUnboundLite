...
qboolean	BG_CanItemBeGrabbed( const entityState_t *ent, const playerState_t *ps ) 
{
	...
	case IT_HOLDABLE:
		if ( item->giTag >= INV_ELECTROBINOCULARS && item->giTag <= INV_FORCE_GEM )
		{
			int	threshold = 5;
			...
			if ( ps->inventory[item->giTag] >= threshold )
			{
				return qfalse;
			}
		}
...
void PlayerStateToEntityState( playerState_t *ps, entityState_t *s )
{
	...
	if ( g_entities[ps->clientNum].client 
		&& g_entities[ps->clientNum].client->NPC_class == CLASS_VEHICLE 
		&& g_entities[ps->clientNum].NPC )
	{
