...
//	}
	...
	{//USE_BUTTON
		...
		{ //we have to be holding the use key in this trigger for x milliseconds before firing
			...
			{ //start the hack
				...
				other->client->ps.hackingTime = level.time + self->hackValue;
				other->client->ps.hackingBaseTime = self->hackValue;
			...
			else if ( !(other->client->usercmd.buttons & BUTTON_USE) )
			{
			...
			{ //hack in progress
				if ( other->client->ps.torsoAnim != BOTH_CONSOLE1 )
				{
					NPC_SetAnim( other, SETANIM_TORSO, BOTH_CONSOLE1, (SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD) );
				}
	...
	{//FIRE_BUTTON
		...
		if( other->s.number == 0 
			&& ( other->client->ps.weapon > MAX_PLAYER_WEAPONS || other->client->ps.weapon <= WP_NONE ) )
	...
	{//USE_BUTTON
		...
		if ( !VectorLengthSquared( other->client->ps.velocity ) 
			&& !PM_CrouchAnim( other->client->ps.legsAnim ) )
		{
...
void SP_trigger_multiple( gentity_t *ent )
{
	...
	G_SpawnInt("usetime", "0", &ent->hackValue);
