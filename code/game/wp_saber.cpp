...
int FP_EntityForceSide( gentity_t *ent )
{
	int forceSide = 0;

	int	lightSide = ent->client->ps.forcePowerLevel[FP_HEAL] + 
		ent->client->ps.forcePowerLevel[FP_TELEPATHY] + 
		ent->client->ps.forcePowerLevel[FP_PROTECT] + 
		ent->client->ps.forcePowerLevel[FP_ABSORB];

	int	darkSide = ent->client->ps.forcePowerLevel[FP_GRIP] + 
		ent->client->ps.forcePowerLevel[FP_LIGHTNING] + 
		ent->client->ps.forcePowerLevel[FP_RAGE] + 
		ent->client->ps.forcePowerLevel[FP_DRAIN];

	if ( ent )
	{
		if ( ent->client || ent->owner )
		{
			if ( lightSide > darkSide )
			{
				forceSide = FORCE_LIGHTSIDE;
			}
			else if ( darkSide > lightSide )
			{
		...
		if ( ent->item )
		{
			if ( ent->item->giType == IT_HOLOCRON )
			{
...
float ForceJumpStrength( gentity_t *self )
{
	int		jumpLevel = self->client->ps.forcePowerLevel[FP_LEVITATION];
	float	jumpStrength = forceJumpStrength[jumpLevel];

	if ( jumpLevel > 3 )
	{
		jumpStrength = (840 + (170 + (80 * (jumpLevel - 1))));

		if ( jumpStrength > Q3_INFINITE )
		{
			jumpStrength = Q3_INFINITE;
		}
	}
...
float ForceJumpHeight( gentity_t *self )
{
	int			jumpLevel = self->client->ps.forcePowerLevel[FP_LEVITATION];
	float		jumpHeight = forceJumpHeight[jumpLevel];
	...
	if ( jumpLevel > 3 )
	{
		...
		case 7:
			...
			break;
		default:
			jumpHeight = (1536 + (32 * (48 + (12 * (jumpLevel - 7)))));

			if ( jumpHeight > Q3_INFINITE )
			{
				jumpHeight = Q3_INFINITE;
			}
			break;
		}
	}
...
float ForcePushPullRadius( gentity_t *self, forcePowers_t forcePower )
{
	int		powerLevel = self->client->ps.forcePowerLevel[forcePower];
	float	radius = forcePushPullRadius[powerLevel];

	if ( powerLevel > 3 )
	{
		radius = (512 + (64 * (powerLevel - 3)));
	}

	return radius;
}

float ForceSpeedDuration( gentity_t *self )
{
	int		speedLevel = self->client->ps.forcePowerLevel[FP_SPEED];
	float	speedDuration = FORCE_SPEED_DURATION;

	speedDuration = (5000.0f * (5000.0f * speedLevel));

	if ( speedDuration > 60000.0f )
	{
		speedDuration = 60000.0f;
	}

	return speedDuration;
}

float ForceSpeedValue( gentity_t *self )
{
	int		speedLevel = self->client->ps.forcePowerLevel[FP_SPEED];
	float	speedValue = forceSpeedValue[speedLevel];
...
float ForceGripDistance( gentity_t *self )
{
	int		gripLevel = self->client->ps.forcePowerLevel[FP_GRIP];
	float	distance = FORCE_GRIP_DIST;

	if ( gripLevel > 5 )
	{
		distance = (FORCE_GRIP_DIST + (64.0f * (gripLevel - 5)));
...
float ForceGripDistanceSquared( gentity_t *self )
{
	float	squaredDistance = (ForceGripDistance( self ) * ForceGripDistance( self ));

	return squaredDistance;
}

float ForceGrip3MinDistance( gentity_t *self )
{
	float	minDistance = (ForceGripDistance( self ) / 4);

	return minDistance;
}

float ForceGrip3MaxDistance( gentity_t *self )
{
	float	maxDistance = (ForceGrip3MinDistance(self) * 2);

	return maxDistance;
}

float ForceRageDuration( gentity_t *self )
{
	int		rageLevel = self->client->ps.forcePowerLevel[FP_RAGE];
	float	rageDuration = FORCE_RAGE_DURATION;
...
int SaberThrowDist( gentity_t *self )
{
	int		throwLevel = self->client->ps.forcePowerLevel[FP_SABERTHROW];
	float	throwDist = saberThrowDist[throwLevel];
	
	if ( throwLevel > 3 )
	{
		throwDist = (400 + (144 * (throwLevel - 3)));
	}
	...
	if ( 1 )
	{
		...
		if ( ent->s.number )
		{
			if ( !ent->client->ps.saberAnimLevel )
			{
				...
				else if ( ent->NPC && ent->client->playerTeam == TEAM_ENEMY )
				{
			...
			for ( int bladeNum = 0; bladeNum < ent->client->ps.saber[saberNum].numBlades; bladeNum++ )
			{
				...
				if ( !didEvent )
				{
					...
					{//just turned on
						...
						if ( ent->s.powerups & ( 1 << PW_CLOAKED ) )
						{
							Jedi_Decloak( ent );
						}
			...
			if ( dmgFraction[i] < saberHitFraction || brokenParry )
			{
				...
				{//actually want to do *some* damage here
					...
					if ( totalDmg[i] > 0 )
					{
						...
						{//never cap the superbreak wins
								...
								&& victim->s.number >= MAX_CLIENTS )
							{
								...
								{//hit a boss character
									int maxDmg = ((3-G_DifficultyLimit())*5)+10;
								...
								{//hit a non-boss saber-user
									int maxDmg = ((3-G_DifficultyLimit())*15)+30;
									if ( totalDmg[i] > maxDmg )
									{
										totalDmg[i] = maxDmg;
									}
								...
								&& ent->NPC )
							{
								...
								{//player hit by a boss character
									int maxDmg = ((G_DifficultyLimit()+1)*4)+3;
									if ( totalDmg[i] > maxDmg )
									{
										totalDmg[i] = maxDmg;
									}
								...
								{//player hit by any enemy //on easy or medium?
									int maxDmg = ((G_DifficultyLimit()+1)*10)+20;
									if ( totalDmg[i] > maxDmg )
									{
										totalDmg[i] = maxDmg;
									}
						...
						int		damage = 0;
						float	skillMult = (1.0f + (0.25f * (ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] - 3)));
						float	rageMult = (1.0f + (0.25f * ent->client->ps.forcePowerLevel[FP_RAGE]));
						...
						float	totalMult = skillMult;

						if ( ent->client->ps.forcePowersActive & ( 1 << FP_RAGE ) )
						{
							totalMult = (skillMult + rageMult);
						}

						if ( !WP_SaberBladeUseSecondBladeStyle( &ent->client->ps.saber[saberNum], bladeNum )
							&& ent->client->ps.saber[saberNum].damageScale != 1.0f )
						{
							damage = (ceil(totalDmg[i]*ent->client->ps.saber[saberNum].damageScale) * totalMult);
						}
						else if ( WP_SaberBladeUseSecondBladeStyle( &ent->client->ps.saber[saberNum], bladeNum )
							&& ent->client->ps.saber[saberNum].damageScale2 != 1.0f )
						{
							damage = (ceil(totalDmg[i]*ent->client->ps.saber[saberNum].damageScale2) * totalMult);
						}
						else
						{
							damage = (ceil(totalDmg[i]) * totalMult);
						}

						if ( damage > Q3_INFINITE )
						{
							damage = Q3_INFINITE;
						}
								...
								int saberNum, int bladeNum )
{
	...
	if ( &g_entities[tr.entityNum] )
	{
		...
		if ( hitEnt->contents & CONTENTS_LIGHTSABER )
		{
			...
			{
				collisionDist = SABER_COLLISION_DIST+6+G_DifficultyLimit()*4;
			}
			{
		...
		for ( i = 0; i < numEnts; i++ )
		{
			...
			{//in range
				...
				{//do damage
					...
					G_Damage( radiusEnts[i], ent, ent, vec3_origin, radiusEnts[i]->currentOrigin, 
						points, DAMAGE_NO_KNOCKBACK, MOD_ENERGY_SPLASH );//MOD_EXPLOSIVE_SPLASH
				}
...
void WP_SaberInFlightReflectCheck( gentity_t *self, usercmd_t *ucmd  )
{
	...
	for ( e = 0 ; e < numListedEntities ; e++ )
	{
		...
		for ( int saberNum = 0; saberNum < numSabers; saberNum++ )
		{
			...
			{
				VectorMA( 
					self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint, 
					self->client->ps.saber[saberNum].blade[bladeNum].length, 
					self->client->ps.saber[saberNum].blade[bladeNum].muzzleDir, 
					tip 
				);
...
{
	int	distSquared = (SaberThrowDist(self) * SaberThrowDist(self));
	...
	*/
	...
	if ( DistanceSquared( self->client->renderInfo.handRPoint, enemy->currentOrigin ) > distSquared )
	{//too far
		return qfalse;
	}
	...
	if ((!InFront(enemy->currentOrigin, self->currentOrigin, self->client->ps.viewangles, 0.0f) || !G_ClearLOS(self, self->client->renderInfo.eyePoint, enemy))
		&& (DistanceHorizontalSquared(enemy->currentOrigin, self->currentOrigin) > distSquared || fabs(enemy->currentOrigin[2] - self->currentOrigin[2]) > (SaberThrowDist(self) - 128)))//65536, 384
...
//closest, most in front... did damage to... took damage from?  How do we know who the player is focusing on?
	...
	int			i, e;
	float		radius = SaberThrowDist( self );//400;
...
void WP_RunSaber( gentity_t *self, gentity_t *saber )
{
	...
	{//home
		...
		{//control if it's returning or just starting
			float	saberSpeed = (SaberThrowDist( self ) + 100);//500;//FIXME: based on force level?
			float	dist;
			...
			if ( self->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2 
				&& self->client->ps.saberEntityState == SES_LEAVING && !enemy )
			{
				if ( dist < (SaberThrowDist( self ) / 2) )
				{
					saberSpeed = SaberThrowDist( self ) - (dist * 2);
				}
			}
			else if ( self->client->ps.saberEntityState == SES_LEAVING && dist < 50 )
			{
				...
				{//auto-tracking an enemy and we can't hit him
					if (saberSpeed < (SaberThrowDist( self ) - 280) )
					{//clamp to a minimum speed
						saberSpeed = (SaberThrowDist( self ) - 280);
					}
				}
			}
			/*
...
{//FIXME: probably need a debounce time
	...
	if ( !noFail )
	{
		...
		{//this is a regular throw, so see if it's legal
			if ( self->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2 )
			{
				if ( !WP_ForcePowerUsable( self, FP_SABERTHROW, (initCost * 2) ) )
				{
					return qfalse;
				}
			}
		...
		gi.trace( &trace, self->client->renderInfo.handRPoint, saberMins, saberMaxs, 
			self->client->renderInfo.handRPoint, saber->s.number, MASK_SOLID, (EG2_Collision)0, 0 );
		...
		gi.trace( &trace, self->currentOrigin, vec3_origin, vec3_origin, 
			self->client->renderInfo.handRPoint, self->s.number, MASK_SOLID, (EG2_Collision)0, 0 );
		...
		{//this is a regular throw, so take force power
			...
			{//at max skill, the cost increases as keep it out
				WP_ForcePowerStart( self, FP_SABERTHROW, initCost );
			}
			else
			{
...
//===FIXME!!!==============================================================================================
	...
	case FORCE_LEVEL_3:
		...
		break;
	case FORCE_LEVEL_4:
		saber->s.apos.trDelta[1] = 1800;
		break;
	case FORCE_LEVEL_5:
	default:
		saber->s.apos.trDelta[1] = (2600 + (800 * (throwLevel - 3)));
		break;
	}
	...
	self->client->ps.saberEntityState = SES_LEAVING;
	self->client->ps.saberEntityDist = SaberThrowDist( self );
...
void WP_SetSaberOrigin( gentity_t *self, vec3_t newOrg )
{
	...
	if ( self->client->ps.saberEntityNum <= 0 || 
		self->client->ps.saberEntityNum >= ENTITYNUM_WORLD )
...
void WP_SaberThrow( gentity_t *self, usercmd_t *ucmd )
{
	...
	{//inflight
		...
		{//see if we're close enough to pick it up
			if ( VectorLengthSquared( saberDiff ) <= SaberThrowDist( self ) )//16 squared//G_BoundsOverlap( self->absmin, self->absmax, saberent->absmin, saberent->absmax ) )//256
			{//caught it
				vec3_t	axisPoint;
				trace_t	trace;
				...
				gi.trace( &trace, axisPoint, vec3_origin, vec3_origin, 
					self->client->renderInfo.handRPoint, self->s.number, MASK_SOLID, (EG2_Collision)0, 0 );
...
void WP_SaberStartMissileBlockCheck( gentity_t *self, usercmd_t *ucmd  )
{
	...
	{//rockettrooper
		...
		if ( Q_irand( 0, 4-(G_DifficultyLimit()*2) ) )
	...
	{//Boba doesn't dodge quite as much
		if ( Q_irand( 0, 2-G_DifficultyLimit()) )
		{//easier level guys do this less
			return;
		}
	}
	...
	if ( incoming )
	{
		...
		else//player
		{
			...
			else
			{
				...
				gi.trace( &trace, start, incoming->mins, incoming->maxs, 
					end, incoming->s.number, MASK_SHOT, G2_COLLIDE, 10 );

				Jedi_DodgeEvasion( self, incoming->owner, &trace, HL_NONE );
			}
			if ( incoming->owner 
				&& incoming->owner->client 
				&& (!self->enemy || self->enemy->s.weapon != WP_SABER) )//keep enemy jedi over shooters
			{
...
{//FIXME: pass in a target ent so we (an NPC) can push/pull just one targeted ent.
	...
	if ( pull )
	{
		...
		radius = ForcePushPullRadius( self, FP_PULL );
	}
	else
	{
		//radius = forcePushPullRadius[self->client->ps.forcePowerLevel[FP_PUSH]];
		radius = ForcePushPullRadius( self, FP_PUSH );
	}
	...
	if ( pull )
	{
		...
		if ( self->client->ps.forcePowerLevel[FP_PULL] > 3 )
		{
			cost /= (self->client->ps.forcePowerLevel[FP_PULL] - 2);
	...
	else
	{
		...
		if ( self->client->ps.forcePowerLevel[FP_PUSH] > 3 )
		{
			cost /= (self->client->ps.forcePowerLevel[FP_PUSH] - 2);
				...
				else
				{
					...
					else
					{
						...
						{//super-hard push
							...
							//knockback *= 5;
							double	pushForce = (1.0 + (0.25 * (self->client->ps.forcePowerLevel[FP_PUSH] - 3)));
...
{
	int	cost = forcePowerNeeded[FP_SPEED];
	...
	{
		cost /= (self->client->ps.forcePowerLevel[FP_SPEED] - 2);
	...
	if ( !WP_ForcePowerUsable( self, FP_SPEED, cost ) )
	{
		return;
	}
	...
	WP_ForcePowerStart( self, FP_SPEED, cost );
...
{
	int		healLevel = self->client->ps.forcePowerLevel[FP_HEAL];
	float	maxHealth = self->client->ps.stats[STAT_MAX_HEALTH];
	...
	if ( self->s.number >= MAX_CLIENTS )
	{
		...
		{
		case 0:
			healAmount = MAX_FORCE_HEAL_HARD;
			break;
		case 1:
			healAmount = MAX_FORCE_HEAL_MEDIUM;
			break;
		case 2:
		default:
			healAmount = MAX_FORCE_HEAL_EASY;
			break;
		}
	}
	...
	if ( g_spskill->integer < 0 )
	{
		...
		{
			if ( healLevel < FORCE_LEVEL_3 )
			{
				healAmount = (100 - (25 * (g_spskill->integer + 1)));

				if ( healAmount > Q3_INFINITE )
				{
					healAmount = Q3_INFINITE;
				}
			}
			else
			{
	...
	else
	{
		if ( maxHealth <= 100 )
		{
			...
			case 0:
				if ( healLevel < FORCE_LEVEL_3 )
				{
					healAmount = floor((float)maxHealth / 1.5f);
				}
				else
				{
					healAmount = floor((float)maxHealth / 2.0f);
				}
				break;
			case 1:
				healAmount = floor((float)maxHealth / 2.0f);
			...
			case 2:
			default:
				healAmount = floor((float)maxHealth / 4.0f);
				break;
			}
...
{
	int		healLevel = self->client->ps.forcePowerLevel[FP_HEAL];
	int		min = 50, max = (min * 4);//FORCE_HEAL_INTERVAL;

	return (healLevel > FORCE_LEVEL_1) ? min : max;
}
...
void ForceHeal( gentity_t *self )
{
	...
	if ( healLevel > FORCE_LEVEL_2 )
	{
		...
		if ( healLevel > FORCE_LEVEL_3 )
		{
			healthMult = (1.0f + (0.25f * (healLevel - 3)));
		}
	}

	float	maxHealth = floor((float)self->client->ps.stats[STAT_MAX_HEALTH] * healthMult);

	if ( self->health <= 0 || maxHealth <= self->health )
	{
		return;
	}

	if ( !WP_ForcePowerUsable( self, FP_HEAL, forceCost ) )
	{//must have enough force power for at least 5 points of health
		return;
	}

	if ( self->painDebounceTime > level.time || 
		(self->client->ps.weaponTime&&self->client->ps.weapon!=WP_NONE) )
	...
	{//instant heal
		...
		if ( max > FP_MaxForceHeal( self ) )
		{//no more than max allowed
			max = FP_MaxForceHeal( self );
		}
		
		if ( max > maxHealth - self->health )
		{//no more than what's missing
			max = maxHealth - self->health;
		}

		float	healMult = (1.0f + (0.25f * (healLevel - 3)));
		float	healAmount = floor((float)max * healMult);
		
		if ( healAmount > maxHealth - self->health )
		{
			healAmount = maxHealth - self->health;
		}
...
void ForceTelepathyRadius( gentity_t *self, gentity_t *traceEnt )
{
	...
	int			telepathyLevel = self->client->ps.forcePowerLevel[FP_TELEPATHY];
	int			levelDuration = (5000 * telepathyLevel);
	...
	float	radius = (512 + (128 * (telepathyLevel - 4))), dist;
	gentity_t* entityList[MAX_GENTITIES];
	...
	for (e = 0; e < numListedEntities; e++)
	{
		gentity_t *radiusEnt = entityList[e];
		...
		if (!radiusEnt->inuse)
			continue;
		if (!radiusEnt->takedamage)
			continue;
		if (OnSameTeam(self, radiusEnt))
			continue;

		for (i = 0; i < 3; i++)
		{
			if (center[i] < radiusEnt->absmin[i])
			{
				v[i] = radiusEnt->absmin[i] - center[i];
			}
			else if (center[i] > radiusEnt->absmax[i])
			{
				v[i] = center[i] - radiusEnt->absmax[i];
			}
			else
			{
				v[i] = 0;
		...
		VectorSubtract(radiusEnt->absmax, radiusEnt->absmin, size);
		VectorMA(radiusEnt->absmin, 0.5, size, ent_org);
		...
		if (!radiusEnt->bmodel && !gi.inPVS(ent_org, traceEnt->client->ps.origin))
		{
			continue;
		}

		gi.trace(&tr, traceEnt->client->ps.origin, vec3_origin, vec3_origin, 
			ent_org, traceEnt->s.number, MASK_SHOT, (EG2_Collision)0, 0);
		
		if (tr.fraction < 1.0f && tr.entityNum != radiusEnt->s.number)
		{
			continue;
		}
		...
		{
			switch (radiusEnt->client->NPC_class)
			{
		...
		if ( targetLive
			...
			&& radiusEnt->health > 0 )
		{
			if ( self->client->playerTeam != radiusEnt->client->playerTeam )
			{
				team_t	saveTeam;
				int		override = 0;

				if ((radiusEnt->NPC->scriptFlags & SCF_NO_MIND_TRICK))
				{
					if (radiusEnt->client->NPC_class == CLASS_GALAKMECH)
					{
						G_AddVoiceEvent(radiusEnt, Q_irand(EV_CONFUSE1, EV_CONFUSE3), Q_irand(3000, 5000));
					}
				}
				else if (radiusEnt->s.weapon != WP_SABER
					&& radiusEnt->client->NPC_class != CLASS_REBORN)
				{
					if ( radiusEnt->s.weapon != WP_NONE
						&& radiusEnt->client->NPC_class != CLASS_TUSKEN
						&& radiusEnt->client->NPC_class != CLASS_NOGHRI
						&& !Pilot_AnyVehiclesRegistered()
						)
					{
						if (radiusEnt->enemy)
						{
							G_ClearEnemy(radiusEnt);
						}

						if (radiusEnt->NPC)
						{
							radiusEnt->client->leader = traceEnt;
						}

						saveTeam = radiusEnt->client->enemyTeam;

						radiusEnt->client->enemyTeam = radiusEnt->client->playerTeam;
						radiusEnt->client->playerTeam = saveTeam;

						radiusEnt->NPC->charmedTime = level.time + levelDuration;

						if (radiusEnt->ghoul2.size() && radiusEnt->headBolt != -1)
						{
							G_PlayEffect(
								G_EffectIndex("force/confusion"),
								radiusEnt->playerModel,
								radiusEnt->headBolt,
								radiusEnt->s.number,
								radiusEnt->currentOrigin,
								levelDuration,
								qtrue
							);
						}
					...
					{
						radiusEnt->NPC->confusionTime = level.time + levelDuration;
						...
						//NPC_PlayConfusionSound(radiusEnt);
...
void ForceTelepathy( gentity_t *self )
{
	...
	gi.trace( &tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, 
		end, self->s.number, MASK_OPAQUE|CONTENTS_BODY, (EG2_Collision)0, 0 );
		...
		{//an enemy
			...
			{//haha!  Jedi aren't easily confused!
				...
				{//just confuse them
					...
					traceEnt->NPC->confusionTime = level.time + levelDuration;//confused for about 10 seconds
	...
	{
		if ( telepathyLevel > FORCE_LEVEL_1 && tr.fraction * 2048 > 64 )
...
{//FIXME: make enemy Jedi able to use this
	...
	{
		initCost /= (self->client->ps.forcePowerLevel[FP_GRIP] - FORCE_LEVEL_2);
	}

	if ( self->client->ps.forcePower < (initCost + 6) )
	...
	AngleVectors( self->client->ps.viewangles, forward, NULL, NULL );
	VectorNormalize( forward );
	VectorMA(self->client->renderInfo.handLPoint, ForceGripDistance( self ), forward, end);//FORCE_GRIP_DIST
	...
	{//I have an enemy
		...
		{//don't auto-pickup guys with keys
			if ( DistanceSquared( self->enemy->currentOrigin, self->currentOrigin ) < ForceGripDistanceSquared( self ) )//FORCE_GRIP_DIST_SQUARED
			{//close enough to grab
				...
				{//need to be facing the enemy
					...
					{//must be in PVS
						gi.trace( &tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, 
							self->enemy->currentOrigin, self->s.number, MASK_SHOT, (EG2_Collision)0, 0 );
	...
	{//okay, trace straight ahead and see what's there
		gi.trace( &tr, self->client->renderInfo.handLPoint, vec3_origin, vec3_origin, 
			end, self->s.number, MASK_SHOT, (EG2_Collision)0, 0 );
...
//rww - RAGDOLL_END
	if ( !traceEnt || 
		traceEnt == self/*???*/ || 
		traceEnt->bmodel || 
		(traceEnt->health <= 0 && traceEnt->takedamage) || 
		(traceEnt->NPC && traceEnt->NPC->scriptFlags & SCF_NO_FORCE) )
	{
		return;
	}
...
//rww - RAGDOLL_END
	...
	if ( traceEnt->client )
	{
		...
		{//can't catch them in mid force jump - FIXME: maybe base it on velocity?
			if ( self->client->ps.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_3 )
			{
				return;
			}
		}
		...
		int	diffInt = (g_spskill->integer - 2);
...
//=CHECKABSORB===
		if ( -1 != WP_AbsorbConversion( traceEnt, traceEnt->client->ps.forcePowerLevel[FP_ABSORB], 
			self, FP_GRIP, self->client->ps.forcePowerLevel[FP_GRIP], forcePowerNeeded[self->client->ps.forcePowerLevel[FP_GRIP]]) )
		{
			if ( self->client->ps.forcePowerLevel[FP_GRIP] <= traceEnt->client->ps.forcePowerLevel[FP_ABSORB] )
			{
...
//===============
	...
	for ( int forcePower = FP_FIRST; forcePower < NUM_FORCE_POWERS; forcePower++ )
	{
		if ( self->client->ps.forcePowersActive & ( 1 << forcePower ) )
		{
			...
			case FP_PROTECT:
			case FP_ABSORB:
				WP_ForcePowerStop( self, (forcePowers_t)forcePower );
				break;
			default:
				break;
			}
	...
	WP_ForcePowerStart( self, FP_GRIP, initCost );
	...
	if ( traceEnt->client )
	{
		...
		{//if we pick up & carry, drop their weap
				...
				)
			{
				...
				else if ( !Q_stricmp( "Rax", traceEnt->NPC_type ) )
				{// so rax can't drop his
				}
...
void ForceLightning( gentity_t *self )
{
	...
	if ( self->client->ps.forcePower < 25 || 
		!WP_ForcePowerUsable( self, FP_LIGHTNING, 0 ) )
	{
		return;
	}
...
{
	int	lightningLevel = self->client->ps.forcePowerLevel[FP_LIGHTNING];
	...
	if ( traceEnt && traceEnt->takedamage )
	{
		if ( !traceEnt->client || 
			...
			traceEnt->enemy == self )
		{//an enemy or object
			int	min = (lightningLevel - 2), max = (min + 2);
			...
			if ( self->client->ps.forcePowersActive & ( 1 << FP_RAGE ) )
			{
				float	rageMult = (0.25f + (0.25f * (self->client->ps.forcePowerLevel[FP_RAGE] - FORCE_LEVEL_1)));

				dmg += floor((float)dmg * rageMult);
			}
			...
			{//Luke, Desann Tavion and Kyle can shield themselves from the attack
				...
				if ( traceEnt->client->ps.groundEntityNum != ENTITYNUM_NONE 
					&& !PM_SpinningSaberAnim( traceEnt->client->ps.legsAnim ) 
					&& !PM_FlippingAnim( traceEnt->client->ps.legsAnim ) 
					&& !PM_RollingAnim( traceEnt->client->ps.legsAnim ) )
				{//if on a surface and not in a spin or flip, play full body resist
					parts = SETANIM_BOTH;
				}
				else
				...
				if ( self->client->ps.forcePowerLevel[FP_LIGHTNING] < (FORCE_LEVEL_4 + diffInt))
				{
			...
			if ( traceEnt 
				&& traceEnt->client 
				&& traceEnt->client->ps.powerups[PW_GALAK_SHIELD] )
			{
				//has shield up
				dmg = 0;
			}
			...
			if (traceEnt->client)
			{
				modPowerLevel = WP_AbsorbConversion(traceEnt, traceEnt->client->ps.forcePowerLevel[FP_ABSORB], 
					self, FP_LIGHTNING, lightningLevel, 1);
			}
			...
			if ( traceEnt->client )
			{
				...
				int	powerup = PW_SHOCKED;

				if ( lightningLevel > FORCE_LEVEL_5 )
				{
					attacker = self;
					powerup = PW_SHOCKED_RED;
				}

				traceEnt->s.powerups |= ( 1 << powerup );

				// If we are dead or we are a bot, we can do the full version
				class_t npc_class = traceEnt->client->NPC_class;
				int		levelDuration = (1000 + (250 * (lightningLevel - 6)));
				...
				case CLASS_SEEKER:
				case CLASS_SENTRY:
					traceEnt->client->ps.powerups[powerup] = level.time + 4000;
					break;
				default:
					if ( traceEnt->health <= 0 )
					{
					...
					{
						if ( powerup == PW_SHOCKED_RED )
						{
							traceEnt->client->ps.powerups[powerup] = level.time + levelDuration;
						}
						else
						{
							traceEnt->client->ps.powerups[powerup] = level.time + 500;
						}
					}
					break;
				}
...
{
	int	drainLevel = self->client->ps.forcePowerLevel[FP_DRAIN];
		...
		{//an enemy or object
			...
			if ( dmg )
			{
				...
				*/
				...
				if ( drainLevel > FORCE_LEVEL_2 )
				{//overcharge health
					float	levelMult = (1.0f + (0.25f * (drainLevel-2)));

					maxHealth = floor( (float)self->client->ps.stats[STAT_MAX_HEALTH] * levelMult );
				}
...
qboolean WP_CheckForceDraineeStopMe( gentity_t *self, gentity_t *drainee )
{
	if ( drainee->NPC
		...
		&& !Q_irand( 0, 100-(drainee->NPC->stats.evasion*10)-(G_DifficultyLimit()*12) ) )
...
void ForceShootDrain( gentity_t *self )
{
	...
	//				continue;
				...
				//in PVS?
				if ( !traceEnt->bmodel 
					&& !gi.inPVS( ent_org, self->client->renderInfo.handLPoint ) )
				{//must be in PVS
					continue;
				}

				//Now check and see if we can actually hit it
				gi.trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, 
					ent_org, self->s.number, MASK_SHOT, G2_RETURNONHIT, 10 );
		...
		{//trace-line
			...
			while ( traces < 10 )
			{//need to loop this in case we hit a Jedi who dodges the shot
				gi.trace( &tr, start, vec3_origin, vec3_origin, 
					end, ignore, MASK_SHOT, G2_RETURNONHIT, 10 );
		...
		self->client->ps.forcePowerDebounce[FP_DRAIN] = level.time + Q_irand( min, max );//200;//so we don't drain so damn fast!
	}
	
	self->client->ps.forcePowerRegenDebounceTime = level.time + 500;

	if ( self->client->ps.forcePowerCostDebounce[FP_DRAIN] < level.time )
	{
		int	drainDebounce = (10 * (drainLevel - 3));

		if ( drainDebounce < 0 )
		{
			drainDebounce = 0;
		}
		else if ( drainDebounce > Q3_INFINITE )
		{
			drainDebounce = Q3_INFINITE;
		}
		...
		self->client->ps.forcePowerCostDebounce[FP_DRAIN] = level.time + drainDebounce;
	}
...
void ForceDrainEnt( gentity_t *self, gentity_t *drainEnt )
{
	...
	if ( self->client->ps.forcePowerDebounce[FP_DRAIN] <= level.time )
	{
		...
		*/
		...
		self->client->ps.forcePowerDebounce[FP_DRAIN] = level.time + Q_irand( min, max );
	}
...
{
	int	cost = forcePowerNeeded[FP_SEE];
	...
	if ( self->client->ps.forcePowerLevel[FP_SEE] > 3 )
	{
		cost /= (self->client->ps.forcePowerLevel[FP_SEE] - 2);
	...
	if ( !WP_ForcePowerUsable( self, FP_SEE, cost ) )
	{
		return;
	}
	...
	WP_ForcePowerStart( self, FP_SEE, cost );
...
{
	int	cost = forcePowerNeeded[FP_PROTECT];
	...
	if ( self->client->ps.forcePowerLevel[FP_PROTECT] > 3 )
	{
		cost /= (self->client->ps.forcePowerLevel[FP_PROTECT] - 2);
	...
	if ( !WP_ForcePowerUsable( self, FP_PROTECT, cost ) )
	{
		return;
	}
	...
	WP_ForcePowerStart( self, FP_PROTECT, cost );
...
{
	int	cost = forcePowerNeeded[FP_ABSORB];
	...
	if ( self->client->ps.forcePowerLevel[FP_ABSORB] > 3 )
	{
		cost /= (self->client->ps.forcePowerLevel[FP_ABSORB] - 2);
	...
	if ( !WP_ForcePowerUsable( self, FP_ABSORB, cost ) )
	{
		return;
	}
	...
	WP_ForcePowerStart( self, FP_ABSORB, cost );
...
{
	int	cost = forcePowerNeeded[FP_RAGE];
	...
	if ( self->client->ps.forcePowerLevel[FP_RAGE] > 3 )
	{
		cost /= (self->client->ps.forcePowerLevel[FP_RAGE] - 2);
	...
	if ( !WP_ForcePowerUsable( self, FP_RAGE, cost ) )
	{
		return;
	}
	...
	WP_ForcePowerStart( self, FP_RAGE, cost );
...
void ForceJumpCharge( gentity_t *self, usercmd_t *ucmd )
{
	...
	if ( self->client->ps.forceJumpCharge > ForceJumpStrength( self ) )
	{
		self->client->ps.forceJumpCharge = ForceJumpStrength( self );
	}
...
void ForceJump( gentity_t *self, usercmd_t *ucmd )
{
	...
	float forceJumpChargeInterval = ForceJumpStrength( self )/(FORCE_JUMP_CHARGE_TIME/FRAMETIME);
...
int WP_AbsorbConversion(gentity_t *attacked, int atdAbsLevel, gentity_t *attacker, int atPower, int atPowerLevel, int atForceSpent)
{
	...
	float	levelMult = (1.0f + (0.25f * (attacked->client->ps.forcePowerLevel[FP_ABSORB] - FORCE_LEVEL_2)));
	...
	float	maxForce = floor((float)attacked->client->ps.forcePowerMax * levelMult);
	...
	if ( attacked->client->ps.forcePower > maxForce )
	{
		attacked->client->ps.forcePower = maxForce;
	}
...
{
	float	maxForce = self->client->ps.forcePowerMax;
	...
	if ( self->client->ps.torsoAnim == BOTH_MEDITATE )
	{
		maxForce = floor((float)self->client->ps.forcePowerMax * 2.0f);
	}

	if ( self->client->ps.forcePower < maxForce )
	{
		if ( overrideAmt )
		{
		...
		if ( self->client->ps.forcePower > maxForce )
		{
			self->client->ps.forcePower = maxForce;
		}
	}
}
...
void WP_ForcePowerStart( gentity_t *self, forcePowers_t forcePower, int overrideAmt )
{
	...
	case FP_SPEED:
		...
		speed = ForceSpeedValue( self );
		duration = ceil(ForceSpeedDuration( self ) * speed);//FIXME: because the timescale scales down (not instant), this doesn't end up being exactly right...
	...
	case FP_RAGE:
		...
		duration = ceil(ForceRageDuration( self ) * speed);//FIXME: because the timescale scales down (not instant), this doesn't end up being exactly right...
	...
	case FP_DRAIN:
		...
		break;
	case FP_PROTECT:
		duration = (5000 + (5000 * self->client->ps.forcePowerLevel[FP_PROTECT]));
		
		if ( duration > Q3_INFINITE )
		{
			duration = Q3_INFINITE;
		}
		...
		break;
	case FP_ABSORB:
		duration = (5000 + (5000 * self->client->ps.forcePowerLevel[FP_ABSORB]));
		...
		break;
	case FP_SEE:
		switch ( self->client->ps.forcePowerLevel[FP_SEE] )
		{
		case 0:
		case 1:
			duration = 5000;
		...
		case 3:
		default:
			duration = (5000 + (5000 * self->client->ps.forcePowerLevel[FP_SEE]));
...
void WP_ForcePowerStop( gentity_t *self, forcePowers_t forcePower )
{
	...
	case FP_SPEED:
		...
		{//player using force speed
			...
			{
				if ( !(self->client->ps.forcePowersActive&(1<<FP_RAGE)) || 
					self->client->ps.forcePowerLevel[FP_RAGE] < FORCE_LEVEL_2 )
...
static void WP_ForcePowerRun( gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd )
{
	...
	case FP_HEAL:
		...
		else if ( self->client->ps.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_3 
		...
		else if ( self->client->ps.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_2 
			&& ( cmd->rightmove || 
				cmd->forwardmove || 
				cmd->upmove > 0 ) )
		...
		{//time to heal again
			...
			{//have available power
				...
				if ( self->s.number < MAX_CLIENTS )
				{
					if ( g_spskill->integer < 0 )
					{
						healAmount *= (3 - g_spskill->integer);
					}
					else
					{
						switch ( g_spskill->integer )
						{
						case 0://easy, heal 3 times as fast...
							healAmount *= 3;
							break;
						case 1://medium, heal twice as fast
							healAmount *= 2;
							break;
						default:
							break;
				...
				WP_ForcePowerDrain( self, forcePower, healAmount );
			}
			else
			{//stop
				WP_ForcePowerStop( self, forcePower );
			...
			{//holding jump in air
				...
				{//still trying to go up
					if ( WP_ForcePowerAvailable( self, FP_LEVITATION, 1 ) )
					{
						...
						{
							int	jumpLevel = self->client->ps.forcePowerLevel[forcePower];
							int	jumpCost = (5 - (jumpLevel - 3));
							int	jumpDebounce = (100 + (100 * (jumpLevel - 3)));
							...
							else
							{
								...
								if ( jumpDebounce > Q3_INFINITE )
								{
									jumpDebounce = Q3_INFINITE;
								}
							}

							WP_ForcePowerDrain( self, FP_LEVITATION, jumpCost );
							self->client->ps.forcePowerDebounce[FP_LEVITATION] = level.time + jumpDebounce;
						}
	...
	case FP_SPEED:
		speed = ForceSpeedValue( self );
		altSpeed = forceSpeedValue[self->client->ps.forcePowerLevel[FP_RAGE]-1];
		...
		{
			int	speedLevel = self->client->ps.forcePowerLevel[forcePower];
			int	speedDebounce = (600 + (150 * (speedLevel - 3)));
			...
			{
				if ( speedDebounce > Q3_INFINITE )
				{
					speedDebounce = Q3_INFINITE;
				}
			}
			...
			self->client->ps.forcePowerCostDebounce[forcePower] = level.time + speedDebounce;
		}
		break;
	case FP_PUSH:
		break;
	...
	case FP_GRIP:
		...
		else if ( self->client->ps.forceGripEntityNum >= 0 
			&& self->client->ps.forceGripEntityNum < ENTITYNUM_WORLD )
		{
			gripEnt = &g_entities[self->client->ps.forceGripEntityNum];
...
//rww - RAGDOLL_END
				...
				&& gripEnt->client->moveType != MT_FLYSWIM )
			{
				...
				&& gripEnt->client->moveType == MT_FLYSWIM 
				&& VectorLengthSquared( gripEnt->client->ps.velocity ) > (300*300) )
			...
			{//boba fett - fly away!
				...
				&& !Q_irand( 0, 100-(gripEnt->NPC->stats.evasion*8)-(G_DifficultyLimit()*20) ) )
			...
			else
			{
				...
				if ( gripEnt->client )
				{
					gripLevel = WP_AbsorbConversion( gripEnt, gripEnt->client->ps.forcePowerLevel[FP_ABSORB], 
						self, FP_GRIP, self->client->ps.forcePowerLevel[FP_GRIP], forcePowerNeeded[gripLevel] );
				}
				...
				{//holding it
					...
					{//we were already playing this anim, we didn't want to restart it, but we want to hold it for at least 100ms, sooo....
				...
				if ( self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_2 &&
					(!InFront( gripEntOrg, self->client->renderInfo.handLPoint, self->client->ps.viewangles, 0.3f ) ||
						DistanceSquared( gripEntOrg, self->client->renderInfo.handLPoint ) > ForceGripDistanceSquared(self)))//FORCE_GRIP_DIST_SQUARED
				{//must face them
					WP_ForcePowerStop( self, FP_GRIP );
					return;
				}
				...
				{//carry
					//cap dist
					if ( dist > ForceGrip3MaxDistance( self ) )
					{
				...
				{//if holding him, make sure there's a clear LOS between my hand and him
					...
					gi.trace( &gripTrace, self->client->renderInfo.handLPoint, NULL, NULL, 
						gripEntOrg, ENTITYNUM_NONE, MASK_FORCE_PUSH, (EG2_Collision)0, 0 );
				...
				if ( gripEnt->client )
				{
					...
					{//level 1 just holds them
						...
						{//level 2 just lifts them
							...
							switch ( self->client->ps.forcePowerLevel[FP_GRIP] )
							{
							case 3:
								levelVel = 12.0f;
								break;
							...
							float gripDist = VectorNormalize( gripEnt->client->ps.velocity )/levelVel;
				...
				if ( self->client->ps.forcePowerDebounce[FP_GRIP] < level.time )
				{
					...
					{//we don't damage the empty vehicle
						...
						int	min = self->client->ps.forcePowerLevel[FP_GRIP], max = (min * 3);
						int gripDmg = Q_irand( min, max );
						int	dflags = (DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK);
						...
						if ( gripLevel != -1 )
						{
							...
							{
							case 1:
								gripDmg = floor((float)gripDmg / 4.0f);
								break;
							case 2:
								gripDmg = floor((float)gripDmg / 2.0f);
								break;
							case 3:
						...
						G_Damage( gripEnt, self, self, dir, gripOrg, gripDmg, dflags, MOD_FORCE_GRIP );//MOD_???
					}
					

					//if ( gripEnt->s.number )
					{
						int	minDmgRate = (150 - (15 * (self->client->ps.forcePowerLevel[FP_GRIP] - 3))), 
							maxDmgRate = (minDmgRate * 4);//750;
						...
						self->client->ps.forcePowerDebounce[FP_GRIP] = level.time + Q_irand( minDmgRate, maxDmgRate );
					}
					/*
					else
					{//player takes damage faster
					...
					*/
					...
					{//no damage at level 1
						int	cost = (3 - (self->client->ps.forcePowerLevel[FP_GRIP] - 3));
						int	minCostRate = 150, maxCostRate = (minCostRate * 4);//750;
						...
						if ( self->client->ps.forcePowerCostDebounce[FP_GRIP] < level.time )
						{
							WP_ForcePowerDrain( self, FP_GRIP, cost );
							self->client->ps.forcePowerCostDebounce[FP_GRIP] = level.time + Q_irand( minCostRate, maxCostRate );
						}
					}
				...
				if ( gripEnt->client && gripEnt->health > 0 )
				{
					...
					switch ( gripEnt->client->ps.weapon )
					{
					case WP_NONE:
					case WP_MELEE:
	...
	case FP_LIGHTNING:
		...
		else
		{
			...
			if ( self->client->ps.forcePowerCostDebounce[forcePower] < level.time )
			{
				int	costDebounce = (10 * (self->client->ps.forcePowerLevel[forcePower] - 3));

				if (costDebounce < 0)
				{
					costDebounce = 0;
				}
				else if (costDebounce > Q3_INFINITE)
				{
					costDebounce = Q3_INFINITE;
				}
				...
				self->client->ps.forcePowerCostDebounce[forcePower] = level.time + costDebounce;
			}
		}
		break;
	//new Jedi Academy force powers
	case FP_RAGE:
		...
		if (self->client->ps.forceRageDrainTime < level.time)
		{
			...
			switch ( self->client->ps.forcePowerLevel[FP_RAGE] )
			{
			case 0:
			case 1:
				addTime = 100;
			...
			default:
				...
				if ( self->client->ps.forcePowerLevel[forcePower] > FORCE_LEVEL_3 )
				{
					self->health -= 1;
				}
				else
		...
		if ( self->client->ps.forcePowerCostDebounce[forcePower] < level.time )
		{
			int	rageLevel = self->client->ps.forcePowerLevel[forcePower];
			int	rageDebounce = (600 + (150 * (rageLevel - 3)));
			...
			{
				if ( rageDebounce > Q3_INFINITE )
				{
					rageDebounce = Q3_INFINITE;
				}
			}
			...
			self->client->ps.forcePowerCostDebounce[forcePower] = level.time + rageDebounce;
		}
		break;
	case FP_DRAIN:
		...
		else if ( self->client->ps.forceDrainEntityNum >= 0 
			&& self->client->ps.forceDrainEntityNum < ENTITYNUM_WORLD )
		{//holding someone
			...
			else
			{
				...
				{//dead ent
					...
					&& drainEnt->client->moveType == MT_FLYSWIM 
					&& VectorLengthSquared( NPC->client->ps.velocity ) > (300*300) )
				{//flying creature broke free
					WP_ForcePowerStop( self, FP_DRAIN );
					return;
				}
				...
				{//boba fett - fly away!
					...
					&& level.time-(self->client->ps.forcePowerDebounce[FP_DRAIN]>self->client->ps.forcePowerLevel[FP_DRAIN]*500)//at level 1, I always get at least 500ms of drain, at level 3 I get 1500ms
					&& !Q_irand( 0, 100-(drainEnt->NPC->stats.evasion*8)-(G_DifficultyLimit()*15) ) )
				{//a jedi who broke free FIXME: maybe have some minimum grip length- a reaction time?
				...
				else
				{
					...
					*/
					...
					{
						G_PlayEffect( 
							G_EffectIndex( "force/drain_hand" ), 
							self->playerModel, 
							self->handLBolt, 
							self->s.number, 
							self->currentOrigin, 
					...
					if ( drainEnt->client )
					{
						...
						trace_t	trace;
						gi.trace( &trace, drainEnt->currentOrigin, drainEnt->mins, drainEnt->maxs, 
							drainEnt->client->ps.forceDrainOrg, drainEnt->s.number, drainEnt->clipmask );
					...
					if ( self->client->ps.forcePowerDebounce[FP_DRAIN] < level.time )
					{
						int drainLevel = WP_AbsorbConversion( drainEnt, drainEnt->client->ps.forcePowerLevel[FP_ABSORB], 
							self, FP_DRAIN, self->client->ps.forcePowerLevel[FP_DRAIN], 0 );
					...
					else
					{
						if ( self->client->ps.forcePowerCostDebounce[FP_DRAIN] < level.time )
						{
							...
							self->client->ps.forcePowerCostDebounce[FP_DRAIN] = level.time + costDebounce;
						}
	...
	case FP_PROTECT:
		if ( self->client->ps.forcePowerCostDebounce[forcePower] < level.time )
		{
			int	protectLevel = self->client->ps.forcePowerLevel[forcePower];
			int	protectDebounce = (300 + (150 * (protectLevel - 3)));
			...
			{
				if ( protectDebounce > Q3_INFINITE )
				{
					protectDebounce = Q3_INFINITE;
				}
			}
			...
			self->client->ps.forcePowerCostDebounce[forcePower] = level.time + protectDebounce;
		}
		break;
	case FP_ABSORB:
		if ( self->client->ps.forcePowerCostDebounce[forcePower] < level.time )
		{
			int	absorbLevel = self->client->ps.forcePowerLevel[forcePower];
			int	absorbDebounce = (600 + (150 * (absorbLevel - 3)));
			...
			{
				if ( absorbDebounce > Q3_INFINITE )
				{
					absorbDebounce = Q3_INFINITE;
				}
			}
			...
			self->client->ps.forcePowerCostDebounce[forcePower] = level.time + absorbDebounce;
		}
		break;
	case FP_SEE:
		if (self->client->ps.forcePowerCostDebounce[forcePower] < level.time)
		{
			int	sightLevel = self->client->ps.forcePowerLevel[forcePower];
			int	sightDebounce = (900 + (150 * (sightLevel - 3)));
			...
			{
				if ( sightDebounce > Q3_INFINITE )
				{
					sightDebounce = Q3_INFINITE;
				}
			}
			...
			self->client->ps.forcePowerCostDebounce[forcePower] = level.time + sightDebounce;
		}
		break;
	default:
		break;
...
void WP_ForcePowersUpdate( gentity_t *self, usercmd_t *ucmd )
{
	...
	{//don't regen force power while throwing saber
		if ( self->client->ps.saberEntityNum < ENTITYNUM_NONE 
			&& self->client->ps.saberEntityNum > 0 )//player is 0
		{//
			if ( &g_entities[self->client->ps.saberEntityNum] != NULL 
				&& g_entities[self->client->ps.saberEntityNum].s.pos.trType == TR_LINEAR )
	...
	{//when not using the force, regenerate at 10 points per second
		if ( self->client->ps.forcePowerRegenDebounceTime < level.time )
		{
			...
			float	regenRate = self->client->ps.forcePowerRegenRate;

			if ( self->client->ps.torsoAnim == BOTH_MEDITATE )
			{
				if ( g_spskill->integer < 0 )
				{
					regenRate = floor((float)regenRate / 4.0f);
				}
				else
				{
					switch ( g_spskill->integer )
					...
					case 0:
						...
						break;
					case 1:
						regenRate = floor((float)regenRate / 2.0f);
						break;
					case 2:
						regenRate = floor((float)regenRate / 1.5f);
						break;
					default:
						break;
					}
			...
			self->client->ps.forcePowerRegenDebounceTime = level.time + regenRate;
...
void WP_InitForcePowers( gentity_t *ent )
{
	...
	{//player
		...
		{
			ent->client->ps.forcePowersKnown = ( 1 << FP_HEAL )|( 1 << FP_LEVITATION )|( 1 << FP_SPEED )|( 1 << FP_PUSH )|
				( 1 << FP_PULL )|( 1 << FP_TELEPATHY )|( 1 << FP_GRIP )|( 1 << FP_LIGHTNING)|
				( 1 << FP_SABERTHROW)|( 1 << FP_SABER_DEFENSE )|( 1 << FP_SABER_OFFENSE )|( 1<< FP_RAGE )|
				( 1<< FP_DRAIN )|( 1<< FP_PROTECT )|( 1<< FP_ABSORB )|( 1<< FP_SEE );
