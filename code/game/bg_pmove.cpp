...
//extern float G_ForceWallJumpStrength( gentity_t *ent );
...
float PM_ForceJumpStrength( pmove_t *pm )
{
	int		jumpLevel = pm->ps->forcePowerLevel[FP_LEVITATION];
...
float PM_ForceWallJumpStrength( pmove_t *pm )
{
	return (PM_ForceJumpStrength( pm ) / 2.5f);
}

float PM_ForceJumpHeight( pmove_t *pm )
{
	int			jumpLevel = pm->ps->forcePowerLevel[FP_LEVITATION];
...
qboolean PM_CheckGrabWall( trace_t *trace )
{
	...
	{//NPCs
		...
		*/
		...
		if ( !PM_PredictJumpSafe( trace->plane.normal, JUMP_OFF_WALL_SPEED, PM_ForceWallJumpStrength( pm ), 1500 ) )
...
static qboolean PM_CheckJump( void )
{
	...
	*/
	...
	if ( (pm->ps->clientNum < MAX_CLIENTS||PM_ControlledByPlayer())
		...
		&& pm->gent->client )
	{
		switch ( pm->gent->client->NPC_class )
		{
		case CLASS_BOBAFETT://player playing as boba fett
		case CLASS_ROCKETTROOPER:
			...
			break;
		default:
			if ( pm->gent->client->ps.inventory[INV_JETPACK] > 0 )
			{
				if ( pm->ps->groundEntityNum == ENTITYNUM_NONE )
				{
					if ( (pm->cmd.buttons & BUTTON_USE) )
					{
						if ( pm->gent->client->jetPackToggleTime < level.time )
						{
							if ( !JET_Flying( pm->gent ) )
							{
								JET_FlyStart( pm->gent );
			...
			{//force jumping && holding jump
				...
				*/
				...
				if ( ( curHeight<=forceJumpHeight[0] ||//still below minimum jump height
					...
					curHeight < PM_ForceJumpHeight( pm ) )//still below maximum jump height
				{//can still go up
					...
					pm->ps->velocity[2] = (PM_ForceJumpHeight( pm ) - curHeight) / PM_ForceJumpHeight( pm ) * PM_ForceJumpStrength( pm );//JUMP_VELOCITY;
					pm->ps->velocity[2] /= 10;
					pm->ps->velocity[2] += JUMP_VELOCITY;
					pm->ps->pm_flags |= PMF_JUMP_HELD;
				}
				else if ( curHeight > forceJumpHeight[0] && curHeight < PM_ForceJumpStrength( pm ) - forceJumpHeight[0] )
							...
							if ( pm->gent && trace.entityNum < ENTITYNUM_WORLD )
							{
								...
								{//push them away and do pain
									...
									G_Damage( 
										...
										damage, 
										DAMAGE_NO_ARMOR | DAMAGE_NO_HIT_LOC | DAMAGE_NO_KNOCKBACK, 
										MOD_MELEE 
									);
									...
									{//didn't kill him
										if ( (traceEnt->s.number==0&&!Q_irand(0,G_DifficultyLimit()))
...
static void PM_FlyMove( void )
{
	...
	{//jetpack accel
		...
		&& ((pm->ps->clientNum < MAX_CLIENTS||PM_ControlledByPlayer()) || 
			(pm->gent 
				&& pm->gent->client 
				&& pm->gent->client->moveType == MT_RUNJUMP)) )
	{
		PM_CheckJump();
		accel = 1.0f;
		pm->ps->velocity[2] -= pm->ps->gravity * pml.frametime;
...
static void PM_AirMove( void ) {
	...
	{//I am force jumping and I'm not holding the button anymore
		...
		float maxJumpHeight = PM_ForceJumpHeight( pm );
...
/*	if ( ( pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE ) && pm->gent->NPC )
		...
		(pm->ps->pm_flags&PMF_TIME_NOFRICTION) ) 
	{
		if ( pm->ps->gravity >= 0 
			&& pm->ps->groundEntityNum != ENTITYNUM_NONE 
			&& !VectorLengthSquared( pm->ps->velocity ) 
...
static void PM_CrashLand( void )
{
	...
	else
	{
		...
		else if ( pm->ps->jumpZStart 
			&& (pm->ps->forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1||(pm->ps->clientNum < MAX_CLIENTS||PM_ControlledByPlayer())) )
		{//we were force-jumping
			...
			{//take off some of it, at least
				...
				float dropAllow = PM_ForceJumpHeight( pm );

				if ( dropAllow < 128 )
				{//always allow a drop from 128, at least
					dropAllow = 128;
				}
				else if ( dropAllow > Q3_INFINITE )
				{
					dropAllow = Q3_INFINITE;
				}
		...
		else
		{
			...
			else
			{
				if ( !PM_AdjustStandAnimForSlope() )
				{
					...
					if ( pm->gent 
						&& pm->gent->client 
						&& pm->gent->client->NPC_class == CLASS_IMPERIAL )
					{
					...
					else
					{
						...
						else
						{
							...
							case WP_BRYAR_PISTOL:
								PM_SetAnim(pm,SETANIM_LEGS,TORSO_WEAPONIDLE2,SETANIM_FLAG_NORMAL);
								break;
							case WP_BLASTER:
							case WP_BOWCASTER:
							case WP_REPEATER:
							...
							case WP_NOGHRI_STICK:
								PM_SetAnim(pm,SETANIM_LEGS,TORSO_WEAPONIDLE3,SETANIM_FLAG_NORMAL);
								break;
							case WP_DISRUPTOR:
							case WP_TUSKEN_RIFLE:
								PM_SetAnim(pm,SETANIM_LEGS,TORSO_WEAPONIDLE4,SETANIM_FLAG_NORMAL);
								break;
							case WP_THERMAL:
								PM_SetAnim(pm,SETANIM_LEGS,TORSO_WEAPONIDLE10,SETANIM_FLAG_NORMAL);
								break;
							case WP_DET_PACK:
								PM_SetAnim(pm,SETANIM_LEGS,BOTH_STAND9,SETANIM_FLAG_NORMAL);
								break;
							case WP_MELEE:
								if ( (pm->gent->client->ps.eFlags & EF_FORCE_GRIPPED) 
									&& pm->gent->client->ps.groundEntityNum != ENTITYNUM_NONE )
								{
									PM_SetAnim(pm,SETANIM_LEGS,BOTH_CONSOLE1,SETANIM_FLAG_NORMAL);
								}
								else
								{
									PM_SetAnim(pm,SETANIM_LEGS,BOTH_STAND6,SETANIM_FLAG_NORMAL);
								}
								break;
							default:
								if ( (pm->gent->client->ps.eFlags & EF_FORCE_GRIPPED) 
		...
		else
		{
			...
			{//running
				...
				else
				{
					if ( pm->gent 
						&& pm->gent->client 
						&& pm->gent->client->NPC_class == CLASS_JAWA )
					{
...
int G_SaberLockStrength( gentity_t *gent )
{
	...
	if ( gent->s.number >= MAX_CLIENTS )
	{
		...
		{
			strength += 5+Q_irand(0,G_DifficultyLimit());
		}
		else
		{
			strength += gent->client->ps.forcePowerLevel[FP_SABER_OFFENSE]+Q_irand(0,G_DifficultyLimit());
			if ( gent->NPC )
			{
	...
	{//player
		strength += gent->client->ps.forcePowerLevel[FP_SABER_OFFENSE]+Q_irand(0,G_DifficultyLimit())+Q_irand(0,1);
	}
	return strength;
}
					...
					*/
					...
					if ( !Q_irand( 0, 2 ) )
					{
						...
						case 1:
							...
							break;
						case 2:
							G_AddVoiceEvent( genemy, Q_irand( EV_COMBAT1, EV_COMBAT3 ), 500 );
							break;
						case 3:
...
float PM_GroundDistance(void)
{
	...
	pm->trace(&tr, pm->ps->origin, pm->mins, pm->maxs, 
		down, pm->ps->clientNum, pm->tracemask, (EG2_Collision)0, 0);
...
float G_GroundDistance(gentity_t *self)
{
	...
	gi.trace(&tr, self->currentOrigin, self->mins, self->maxs, 
		down, self->s.number, self->clipmask, (EG2_Collision)0, 0);
		...
		*/
		...
		{//can't attack for twice whatever your skill level's parry debounce time is
			...
			{//player
				...
				{
					int	playerParryTime = parryDebounce[pm->ps->forcePowerLevel[FP_SABER_DEFENSE]];

					if ( pm->ps->forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_3 )
					{
						playerParryTime = 50;
					}

					pm->ps->weaponTime = playerParryTime * 2;
				}
			}
			else
			{//NPC
				...
				{//WTF???
					int	npcParryTime = parryDebounce[pm->ps->forcePowerLevel[FP_SABER_DEFENSE]];
					...
					pm->ps->weaponTime = npcParryTime * 2;
				}
			}
		}
		switch ( pm->ps->saberBlocked )
...
qboolean PM_NPCCheckAttackRoll( void )
{
	if ( pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()//NPC
		...
		&& ( pm->gent->NPC->rank > RANK_CREWMAN && !Q_irand(0,3-G_DifficultyLimit()) )
		&& pm->gent->enemy
...
//				pm->gent->client->saberTrail.duration = 75; // saber trail lasts for 75ms...feel free to change this if you want it longer or shorter
	...
	if (pm->ps->forcePowersActive & (1 << FP_RAGE))
	{
		switch ( pm->ps->forcePowerLevel[FP_RAGE] )
		{
		case 1:
			addTime *= 0.9;
			break;
