...
void Mark1Dead_FireRocket (void)
{
	...
	int		damage = BOWCASTER_DAMAGE;//50;
	...
	{
		velocity = floor((float)BOWCASTER_VELOCITY * (1.0f + (0.1f * (g_spskill->integer - 2))));
	...
	gentity_t *missile = CreateMissile( muzzle1, muzzle_dir, velocity, 10000, NPC );
...
void Mark1Dead_FireBlaster (void)
{
	...
	mdxaBone_t	boltMatrix;
	int			bolt;
	int			damage = REPEATER_NPC_DAMAGE_HARD;
	float		velocity = 1600;
	...
	if ( g_spskill->integer > 2 )
	{
		velocity = floor((float)1600 * (1.0f + (0.1f * (g_spskill->integer - 2))));
	...
	missile = CreateMissile( muzzle1, muzzle_dir, velocity, 10000, NPC );
...
void Mark1_FireBlaster(void)
{
	...
	mdxaBone_t	boltMatrix;
	int			bolt;
	int			damage = REPEATER_NPC_DAMAGE_HARD;//1;
	float		velocity = 1600;
...
//	G_PlayEffect( "blaster/muzzle_flash", muzzle1 );
	...
	gentity_t *missile = CreateMissile( muzzle1, forward, velocity, 10000, NPC );
