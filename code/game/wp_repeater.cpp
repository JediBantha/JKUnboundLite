...
//---------------------------------------------------------
{
	...
	if ( ent->s.number != 0 )
	{
		...
		{
			velocity = floor((float)REPEATER_VELOCITY * (1.0f + (0.1f * (g_spskill->integer - 2))));
...
//---------------------------------------------------------
{
	...
	if ( ent->s.number != 0 )
	{
		if ( g_spskill->integer > 2 )
		{
			velocity = floor((float)REPEATER_ALT_VELOCITY * (1.0f + (0.1f * (g_spskill->integer - 2))));
	...
	else
	{
		...
		missile = CreateMissile( start, forwardVec, velocity, 10000, ent, qtrue );
	}
