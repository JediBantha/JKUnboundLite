...
qboolean NPC_CheckLookTarget( gentity_t *self )
{
	if ( self->client )
	{
		...
		{//within valid range
			...
			else if ( g_entities[self->client->renderInfo.lookTarget].client 
				&& self->enemy && (&g_entities[self->client->renderInfo.lookTarget] != self->enemy) )
...
qboolean Player_Flying( gentity_t *self )
{
	return ((qboolean)(self->client->moveType == MT_FLYSWIM));
}

void Player_FlyStart( gentity_t *self )
{
	if ( self->s.powerups & ( 1 << PW_CLOAKED ) )
	{
		Jedi_Decloak( self );
	}

	if ( !Player_Flying( self ) )
	{
...
void Player_FlyStop( gentity_t *self )
{
...
qboolean JET_Flying( gentity_t *self )
{
	...
	if (self->client->ps.inventory[INV_JETPACK] > 0)
	{
		return (Player_Flying(self));
	}
...
void JET_FlyStart( gentity_t *self )
{
	...
	if ( !self->s.number 
		&& self->client->jetPackToggleTime >= level.time )
	{
		return;
	}
	...
	self->client->jetPackToggleTime = level.time + JETPACK_TOGGLE_TIME;
}
