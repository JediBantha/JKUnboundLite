...
static cvarTable_t cvarTable[] = {
	...
	{ &cg_drawShieldWarning, "cg_drawShieldWarning", "1", CVAR_ARCHIVE  },
...
static void CG_RegisterGraphics( void ) {
	...
	cgs.media.boltShader				= cgi_R_RegisterShader( "gfx/misc/blueLine_old" );
	cgs.media.boltShaderRed				= cgi_R_RegisterShader( "gfx/misc/l6_redLine" );
...
//	cgs.media.noammoShader			= cgi_R_RegisterShaderNoMip( "gfx/hud/noammo");
	...
	cgs.media.DPLightForcePowerOverlay = cgi_R_RegisterShader("gfx/hud/force_swirl_light");
	cgs.media.DPSaberForcePowerOverlay = cgi_R_RegisterShader("gfx/hud/force_swirl_saber");
	cgs.media.DPDarkForcePowerOverlay = cgi_R_RegisterShader("gfx/hud/force_swirl_dark");
...
*/
	...
	cgs.media.electricBodyShaderRed			= cgi_R_RegisterShader( "gfx/misc/redelectric" );
	cgs.media.electricBody2ShaderRed		= cgi_R_RegisterShader( "gfx/misc/redfullbodyelectric2" );
...
//	int				tag;
	float			addX;
	vec4_t			textColor = { .278f, .784f, .573f, 1.0f };
	char			text[1024]={0};
	...
	if (!count)
	{
		...
		CG_DrawProportionalString( 
			...
			y2 + 22, 
			text, 
			CG_CENTER | CG_SMALLFONT, 
			colorTable[CT_ICON_GREEN] 
		);
	...
	if (inv_icons[cg.inventorySelect])
	{
		...
		CG_DrawPic( 
			x-(bigIconSize/2), 
			(y-((bigIconSize-smallIconSize)/2))+10, 
			bigIconSize, 
			bigIconSize, 
		...
		if (inv_names[cg.inventorySelect])
		{
				...
				&& item->classname[0] )
			{
				...
				Com_sprintf( itemName, sizeof(itemName), file,	item->classname );	//SP_INGAME_%s
...
const char *inventoryDesc[MAX_INVENTORY] =
{
"INGAME_NEURO_SAAV_DESC",
"INGAME2_BACTA_DESC",
"INGAME_INQUISITOR_DESC",
"INGAME_LA_GOGGLES_DESC",
"INGAME_PORTABLE_SENTRY_DESC",
"INGAME2_JETPACK_DESC",
"INGAME2_CLOAK_DESC",
"INGAME2_FORCE_GEM_DESC",
"INGAME_GOODIE_KEY_DESC",
"INGAME_SECURITY_KEY_DP_DESC",
};
...
void CG_DrawDataPadInventorySelect( void )
{
	...
	if (!count)
	{
		...
		int w = cgi_R_Font_StrLenPixels( text, cgs.media.qhFontSmall, 1.0f );
		int x = ( SCREEN_WIDTH - w ) / 2;
		CG_DrawProportionalString(x, 300 + 22, text, CG_CENTER | CG_SMALLFONT, colorTable[CT_ICON_GREEN]);
		return;
	}
	...
	if (inv_icons[cg.DataPadInventorySelect])
	{
		...
		CG_DrawPic( 
			...
			(graphicYPos-((bigIconSize-smallIconSize)/2))+10, 
			bigIconSize, 
			bigIconSize, 
	...
	if ((cg.DataPadInventorySelect>=0) && (cg.DataPadInventorySelect<18))//13	//(INV_GOODIE_KEY+10)
	{
		cgi_SP_GetStringTextString( va( "SP_%s", inventoryDesc[cg.DataPadInventorySelect]), text, sizeof(text));	//SP_INGAME_%s
		...
		if (text[0])
		{
			...
			CG_DisplayBoxedText( 
				...
				textboxHeight,//300, 
				text,
				4,//cgs.media.qhFontSmall,
				textScale,//0.7f,
				colorTable[CT_WHITE]//textColor
			);
		}
	}
}
...
{
	"SP_ABSORB_INGAME",//"SP_INGAME_ABSORB2",
	"SP_HEAL_INGAME",//"SP_INGAME_HEAL2",
	"SP_PROTECT_INGAME",//"SP_INGAME_PROTECT2",
	"SP_TELEPATHY_INGAME",//"SP_INGAME_MINDTRICK2",

	"SP_SPEED_INGAME",//"SP_INGAME_SPEED2",
	"SP_PUSH_INGAME",//"SP_INGAME_PUSH2",
	"SP_PULL_INGAME",//"SP_INGAME_PULL2",
	"SP_SENSE_INGAME",//"SP_INGAME_SEEING2",

	"SP_DRAIN_INGAME",//"SP_INGAME_DRAIN2",
	"SP_LIGHTNING_INGAME",//"SP_INGAME_LIGHTNING2",
	"SP_RAGE_INGAME",//"SP_INGAME_DARK_RAGE2",
	"SP_GRIP_INGAME",//"SP_INGAME_GRIP2",
};
...
*/
	...
	if (force_icons[showPowers[cg.forcepowerSelect]])
	{
		CG_DrawPic( 
			x-(bigIconSize/2), 
			(y-((bigIconSize-smallIconSize)/2)) + yOffset, 
			bigIconSize, 
			bigIconSize, 
	...
	{
		int	displayColor = CT_ICON_BLUE;
		int w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
		int x = ( SCREEN_WIDTH - w ) / 2;

		switch ( cg.forcepowerSelect )
		{
		case 0://FP_ABSORB
		case 1://FP_HEAL
		case 2://FP_PROTECT
		case 3://FP_TELEPATHY
			displayColor = CT_FORCE_LIGHT;
			break;
		case 8://FP_DRAIN
		case 9://FP_LIGHTNIGN
		case 10://FP_RAGE
		case 11://FP_GRIP
			displayColor = CT_FORCE_DARK;
			break;
		default:
			break;
		}
		...
		cgi_R_Font_DrawString( 
			x, 
			(SCREEN_HEIGHT - 24) + yOffset, 
			text, 
			colorTable[displayColor],
...
{
"ABSORB_DESC",//"FORCE_ABSORB_DESC",
"HEAL_DESC",//"FORCE_HEAL_DESC",
"PROTECT_DESC",//"FORCE_PROTECT_DESC",
"TELEPATHY_DESC",//"FORCE_MIND_TRICK_DESC",

"JUMP_DESC",//"FORCE_JUMP_DESC",
"SPEED_DESC",//"FORCE_SPEED_DESC",
"PUSH_DESC",//"FORCE_PUSH_DESC",
"PULL_DESC",//"FORCE_PULL_DESC",
"SENSE_DESC",//"FORCE_SENSE_DESC",

"SABER_THROW_DESC",//"FORCE_SABER_THROW_DESC",
"SABER_DEFENSE_DESC",//"FORCE_SABER_DEFENSE_DESC",
"SABER_OFFENSE_DESC",//"FORCE_SABER_OFFENSE_DESC",

"DRAIN_DESC",//"FORCE_DRAIN_DESC",
"LIGHTNING_DESC",//"FORCE_LIGHTNING_DESC",
"RAGE_DESC",//"FORCE_RAGE_DESC",
"GRIP_DESC",//"FORCE_GRIP_DESC",
};

const char *forcepowerLvl1Desc[NUM_FORCE_POWERS] =
{
"ABSORB_LVL1_DESC",//"FORCE_ABSORB_LVL1_DESC",
"HEAL_LVL1_DESC",//"FORCE_HEAL_LVL1_DESC",
"PROTECT_LVL1_DESC",//"FORCE_PROTECT_LVL1_DESC",
"TELEPATHY_LVL1_DESC",//"FORCE_MIND_TRICK_LVL1_DESC",

"JUMP_LVL1_DESC",//"FORCE_JUMP_LVL1_DESC",
"SPEED_LVL1_DESC",//"FORCE_SPEED_LVL1_DESC",
"PUSH_LVL1_DESC",//"FORCE_PUSH_LVL1_DESC",
"PULL_LVL1_DESC",//"FORCE_PULL_LVL1_DESC",
"SENSE_LVL1_DESC",//"FORCE_SENSE_LVL1_DESC",

"SABER_THROW_LVL1_DESC",//"FORCE_SABER_THROW_LVL1_DESC",
"SABER_DEFENSE_LVL1_DESC",//"FORCE_SABER_DEFENSE_LVL1_DESC",
"SABER_OFFENSE_LVL1_DESC",//"FORCE_SABER_OFFENSE_LVL1_DESC",

"DRAIN_LVL1_DESC",//"FORCE_DRAIN_LVL1_DESC",
"LIGHTNING_LVL1_DESC",//"FORCE_LIGHTNING_LVL1_DESC",
"RAGE_LVL1_DESC",//"FORCE_RAGE_LVL1_DESC",
"GRIP_LVL1_DESC",//"FORCE_GRIP_LVL1_DESC",
};

const char *forcepowerLvl2Desc[NUM_FORCE_POWERS] =
{
"ABSORB_LVL2_DESC",//"FORCE_ABSORB_LVL2_DESC",
"HEAL_LVL2_DESC",//"FORCE_HEAL_LVL2_DESC",
"PROTECT_LVL2_DESC",//"FORCE_PROTECT_LVL2_DESC",
"TELEPATHY_LVL2_DESC",//"FORCE_MIND_TRICK_LVL2_DESC",

"JUMP_LVL2_DESC",//"FORCE_JUMP_LVL2_DESC",
"SPEED_LVL2_DESC",//"FORCE_SPEED_LVL2_DESC",
"PUSH_LVL2_DESC",//"FORCE_PUSH_LVL2_DESC",
"PULL_LVL2_DESC",//"FORCE_PULL_LVL2_DESC",
"SENSE_LVL2_DESC",//"FORCE_SENSE_LVL2_DESC",

"SABER_THROW_LVL2_DESC",//"FORCE_SABER_THROW_LVL2_DESC",
"SABER_DEFENSE_LVL2_DESC",//"FORCE_SABER_DEFENSE_LVL2_DESC",
"SABER_OFFENSE_LVL2_DESC",//"FORCE_SABER_OFFENSE_LVL2_DESC",

"DRAIN_LVL2_DESC",//"FORCE_DRAIN_LVL2_DESC",
"LIGHTNING_LVL2_DESC",//"FORCE_LIGHTNING_LVL2_DESC",
"RAGE_LVL2_DESC",//"FORCE_RAGE_LVL2_DESC",
"GRIP_LVL2_DESC",//"FORCE_GRIP_LVL2_DESC",
};

const char *forcepowerLvl3Desc[NUM_FORCE_POWERS] =
{
"ABSORB_LVL3_DESC",//"FORCE_ABSORB_LVL3_DESC",
"HEAL_LVL3_DESC",//"FORCE_HEAL_LVL3_DESC",
"PROTECT_LVL3_DESC",//"FORCE_PROTECT_LVL3_DESC",
"TELEPATHY_LVL3_DESC",//"FORCE_MIND_TRICK_LVL3_DESC",

"JUMP_LVL3_DESC",//"FORCE_JUMP_LVL3_DESC",
"SPEED_LVL3_DESC",//"FORCE_SPEED_LVL3_DESC",
"PUSH_LVL3_DESC",//"FORCE_PUSH_LVL3_DESC",
"PULL_LVL3_DESC",//"FORCE_PULL_LVL3_DESC",
"SENSE_LVL3_DESC",//"FORCE_SENSE_LVL3_DESC",

"SABER_THROW_LVL3_DESC",//"FORCE_SABER_THROW_LVL3_DESC",
"SABER_DEFENSE_LVL3_DESC",//"FORCE_SABER_DEFENSE_LVL3_DESC",
"SABER_OFFENSE_LVL3_DESC",//"FORCE_SABER_OFFENSE_LVL3_DESC",

"DRAIN_LVL3_DESC",//"FORCE_DRAIN_LVL3_DESC",
"LIGHTNING_LVL3_DESC",//"FORCE_LIGHTNING_LVL3_DESC",
"RAGE_LVL3_DESC",//"FORCE_RAGE_LVL3_DESC",
"GRIP_LVL3_DESC",//"FORCE_GRIP_LVL3_DESC",
};

const char *ForcePowerLevelDesc( int forcePower )
{
	char		text[512] = {NULL};

	forcePower = cg.DataPadforcepowerSelect;
	
	int			powerLevel = player->client->ps.forcePowerLevel[forcePower];
	const char	*levelDesc = "NULL";

	switch ( cg.DataPadforcepowerSelect )
	{
	case 0:
		{
			int			absorbDuration = (5 + (5 * powerLevel));
			int			absorbPercent = (15 + (15 * (powerLevel - 3)));

			if ( absorbDuration > Q3_INFINITE )
			{
				absorbDuration = Q3_INFINITE;
			}
			...
			levelDesc = va( "Rank %i - For %i seconds, Jedi can absorb a great amount of Force energy directed at them with no ill effects and gain a large amount of Force power back as a result.\nEnergy Absorption: %i Percent", powerLevel, absorbDuration, absorbPercent );
		}
		break;
	case 1:
		{
			int	levelPercent = (100 + (25 * (powerLevel - 3)));
			
			levelDesc = va( "Rank %i - Jedi heals instantaneously as with rank %i. Healing amount depends on game difficulty.\nHealing: %i Percent\nOvercharge: %i Percent", powerLevel, (powerLevel - 1), levelPercent, levelPercent );
		}
		break;
	case 2:
		{
			int			protectDuration = (5 + (5 * powerLevel));
			const char	*protectPercent = "no amount of Force energy";

			if ( protectDuration > Q3_INFINITE )
			{
				protectDuration = Q3_INFINITE;
			}

			switch ( powerLevel )
			{
			case 4:
				protectPercent = "twenty-five percent of the damage";
				break;
			case 5:
				protectPercent = "ten percent of the damage";
				break;
			case 6:
				protectPercent = "five percent of the damage";
				break;
			case 7:
				protectPercent = "only one percent of the damage";
				break;
			default:
				break;
			}
			
			levelDesc = va( "Rank %i - For %i seconds, Jedi is fully protected against all forms of damage (with the exception of Force based attacks), with %s being extracted from their Force reserves.", powerLevel, protectDuration, protectPercent );
		}
		break;
	case 3:
		{
			int	charmRange = (256 + (64 * (powerLevel - 4)));
			int	charmDuration = (5 * powerLevel);
			...
			if ( powerLevel < 9 )
			{
				levelDesc = va( "Rank %i - Jedi can turn enemies within %i centimeters of the target into allies for %i seconds, or distract enemies that have not noticed them by using mind trick on a nearby surface. An ally will not respond to direct attacks by the Jedi.", powerLevel, charmRange, charmDuration );
			}
			else
			{
	...
	case 4:
		{
			int			levelHeight = (48 + (12 * (powerLevel - 7)));

			if ( levelHeight > Q3_INFINITE )
			{
				levelHeight = Q3_INFINITE;
			}
			...
			levelDesc = va( "Rank %i - Jedi can leap up to %i times their normal height.", powerLevel, levelHeight );
		}
		break;
	case 5:
		{
			int	speedBoost = (150 + (10 * (powerLevel - 5)));
			int	speedDuration = (5 + (5 * powerLevel));

			if ( speedBoost > Q3_INFINITE )
			{
				speedBoost = Q3_INFINITE;
			}

			if ( speedDuration > Q3_INFINITE )
			{
				speedDuration = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi moves faster than normal for %i seconds.\nPhysical Speed: %i Percent\nMental Speed: 175 Percent", powerLevel, speedDuration, speedBoost );
		}
		break;
	case 6:
		{
			int	pushStrength = (100 + (25 * (powerLevel - 3)));
			int	pushRange = (384 + (64 * powerLevel));

			if ( pushStrength > Q3_INFINITE )
			{
				pushStrength = Q3_INFINITE;
			}

			if ( pushRange > Q3_INFINITE )
			{
				pushRange = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi can push back multiple enemies with enough force to do damage by pushing the enemies into solid surfaces or off ledges. They can also use it to attempt to break saber locks.\nPush Range: %i Centimeters\nPush Strength: %i Percent", powerLevel, pushRange, pushStrength );
		}
		break;
	case 7:
		{
			int	pullRange = (384 + (64 * powerLevel));

			if ( pullRange > Q3_INFINITE )
			{
				pullRange = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi can pull multiple enemies and their weapons.\nPull Range: %i Centimeters", powerLevel, pullRange );
		}
		break;
	case 8:
		{
			int	senseDuration = (20 + (5 * (powerLevel - 3)));

			if ( senseDuration > Q3_INFINITE )
			{
				senseDuration = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi can sense people, objects, invisible markers, and the health of living beings at long range for %i seconds.", powerLevel, senseDuration );
		}
		break;
	case 9:
		{	
			int	throwRange = (400 + (144 * (powerLevel - 3)));

			if ( throwRange > Q3_INFINITE )
			{
				throwRange = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi will automatically steer the saber into nearby enemies as with rank %i.\nRange: %i Centimeters", powerLevel, (powerLevel - 1), throwRange );
		}
		break;
	case 10:
		{
			levelDesc = va( "Rank %i - Jedi has a larger defense area and can deflect projectiles back at the attacker as with rank %i. Increased deflection speed.", powerLevel, (powerLevel - 1) );
		}
		break;
	case 11:
		{
			int	skillDamage = (100 + (25 * (powerLevel - 3)));

			if ( skillDamage > Q3_INFINITE )
			{
				skillDamage = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi is proficient in the use of a lightsaber and has a chance to break saber locks and parrys against other Jedi of similar proficiency and will break saber locks and parrys against weaker opponents as with rank %i.\nLightsaber Damage: %i Percent", powerLevel, (powerLevel - 1), skillDamage );
		}
		break;
	case 12:
		{
			int	min = powerLevel, max = (min * 2);
			int	overchargePercent = (25 + (25 * (powerLevel - 3)));
			...
			if ( overchargePercent > Q3_INFINITE )
			{
				overchargePercent = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - Jedi can drain life essence from another person and transfer it to themselves by making direct physical contact with them or from a safe distance away.  As with rank %i, Multiple people can be drained at one time from a distance.\nDamage: %i-%i\nOvercharge: %i Percent", powerLevel, (powerLevel - 1), min, max, overchargePercent );
		}
		break;
	case 13:
		{
			int	min = (powerLevel - 2), max = (min + 2);

			if ( powerLevel < 6 )
			{
				levelDesc = va( "Rank %i - Jedi emanates a sustained fan of electricity in a large forward arc, damaging anything in front of them as with rank %i.\nDamage: %i-%i", powerLevel, (powerLevel - 1), min, max );
			}
			else
			{
				int	resMin = (powerLevel - 5), resMax = (resMin + 2);

				if ( resMin > Q3_INFINITE )
				{
					resMin = Q3_INFINITE;
				}

				if ( resMax > Q3_INFINITE )
				{
					resMax = Q3_INFINITE;
				}

				levelDesc = va( "Rank %i - Jedi has fully mastered the use of Sith Lightning, emanating a sustained, red-hued fan of electricity in a large forward arc, damaging anything in front of them. Victims hit by Sith Lightning continue to take damage over time.\nDamage: %i-%i\nResidual Damage = %i-%i", powerLevel, min, max, resMin, resMax );
			}
		}
		break;
	case 14:
		{
			...
			if ( ragePercent > Q3_INFINITE )
			{
				ragePercent = Q3_INFINITE;
			}

			levelDesc = va( "Rank %i - For %i seconds, the Jedi is protected against %i percent of all damage, is %i percent faster, has %i percent stronger attacks, and cannot die. The Jedi loses only a small amount of health but still must wait for %i seconds after the rage before it can be used again. The Jedi must have at least %i health to use this power.", powerLevel, rageDuration, 90, 50, ragePercent, 10, 25 );
		}
		break;
	case 15:
		{
			int			gripDmg = (powerLevel * 3);
			int			minRate = (150 - (15 * (powerLevel - 3))), maxRate = (minRate * 4);
			int			gripRange = (512 + (64 * (powerLevel - 5)));
			const char	*gripSpeed = "Fast";
			...
			{
				levelDesc = va( "Rank %i - Jedi can lift one enemy off the ground, do %i-%i damage, and smash the enemy into surfaces or use the enemy as a shield as with rank %i.\nDamage Rate: %i-%i Milliseconds\nMovement Speed: %s\nRange = %i Centimeters", powerLevel, powerLevel, gripDmg, (powerLevel - 1), minRate, maxRate, gripSpeed, gripRange );
			}
		}
		break;
	default:
...
//	memcpy(fadeColor, colorTable[CT_WHITE], sizeof(vec4_t));
	...
	for (iconCnt=1;iconCnt<(sideLeftIconCnt+1);i--)
	{
		...
		{
			int	overlay = cgs.media.DPForcePowerOverlay;

			switch (showDataPadPowers[i])
			{
			case FP_HEAL:
			case FP_TELEPATHY:
			case FP_PROTECT:
			case FP_ABSORB:
				overlay = cgs.media.DPLightForcePowerOverlay;
				break;
			case FP_GRIP:
			case FP_LIGHTNING:
			case FP_RAGE:
			case FP_DRAIN:
				overlay = cgs.media.DPDarkForcePowerOverlay;
				break;
			case FP_SABERTHROW:
			case FP_SABER_DEFENSE:
			case FP_SABER_OFFENSE:
				overlay = cgs.media.DPSaberForcePowerOverlay;
				break;
			default:
				break;
			}
	...
	if (force_icons[showDataPadPowers[cg.DataPadforcepowerSelect]])
	{
			...
			((cg_updatedDataPadForcePower3.integer - 1) == showDataPadPowers[cg.DataPadforcepowerSelect]))
		{
			...
			switch (showDataPadPowers[cg.DataPadforcepowerSelect])
			{
			case FP_HEAL:
			case FP_TELEPATHY:
			case FP_PROTECT:
			...
			CG_DrawPic( centerXPos-(bigIconSize/2), (graphicYPos-((bigIconSize-smallIconSize)/2)), bigIconSize, bigIconSize, overlay );
		}
	}
	...
	cgi_SP_GetStringTextString( va("SP_%s",forcepowerDesc[cg.DataPadforcepowerSelect]), text, sizeof(text) );

	switch ( player->client->ps.forcePowerLevel[showDataPadPowers[cg.DataPadforcepowerSelect]] )
	{
	case 1:
		cgi_SP_GetStringTextString( va("SP_%s",forcepowerLvl1Desc[cg.DataPadforcepowerSelect]), text2, sizeof(text2) );
		break;
	case 2:
		cgi_SP_GetStringTextString( va("SP_%s",forcepowerLvl2Desc[cg.DataPadforcepowerSelect]), text2, sizeof(text2) );
		break;
	case 3:
		cgi_SP_GetStringTextString( va("SP_%s",forcepowerLvl3Desc[cg.DataPadforcepowerSelect]), text2, sizeof(text2) );
		break;
	default:
		cgi_SP_GetStringTextString( va( "%s", ForcePowerLevelDesc( cg.DataPadforcepowerSelect )), text2, sizeof(text2) );
		break;
	}
