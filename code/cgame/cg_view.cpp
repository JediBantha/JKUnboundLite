...
void CG_TestG2Model_f (void) 
{
	...
	cg.testModel = gi.G2API_InitGhoul2Model(*((CGhoul2Info_v *)cg.testModelEntity.ghoul2), 
		cg.testModelName, 
...
void CG_TestModelSetAnglespre_f(void)
{
	...
	gi.G2API_SetBoneAngles(&ghoul2[cg.testModel], CG_Argv(1), angles, 
		BONE_ANGLES_PREMULT, POSITIVE_X, POSITIVE_Z, 
		POSITIVE_Y, NULL, 0, 0);
}
...
void CG_TestModelSetAnglespost_f(void)
{
	...
	gi.G2API_SetBoneAngles(&ghoul2[cg.testModel], CG_Argv(1), angles, 
		BONE_ANGLES_POSTMULT, POSITIVE_X, POSITIVE_Z, 
		POSITIVE_Y, NULL, 0, 0);
}
...
void CG_TestModelAnimate_f(void)
{
	...
	gi.G2API_SetBoneAnim(&ghoul2[cg.testModel], boneName, atoi(CG_Argv(2)), atoi(CG_Argv(3)), 
		BONE_ANIM_OVERRIDE_LOOP, atof(CG_Argv(4)), cg.time, -1, -1);
...
static void CG_CalcIdealThirdPersonViewLocation(void)
{
	...
	{//stay back
		VectorMA( 
			cg_entities[cg_entities[cg.snap->ps.clientNum].gent->activator->s.number].lerpOrigin, 
			-180.0f, 
			camerafwd, 
			cameraIdealLoc 
		);
		...
		&& player->client->ps.forcePowerDuration[FP_SPEED] )
	{
		...
		float length = ForceSpeedDuration( player )*ForceSpeedValue( player );
		float amt = forceSpeedRangeMod[player->client->ps.forcePowerLevel[FP_SPEED]];

		if ( player->client->ps.forcePowerLevel[FP_SPEED] > FORCE_LEVEL_3 )
		{
			amt = 60.0f;
		}
...
static qboolean	CG_CalcFov( void ) 
{
	...
	else if ( (!cg.zoomMode || cg.zoomMode > 2) 
		&& (cg.snap->ps.forcePowersActive&(1<<FP_SPEED)) 
