...
void CG_ItemPickup( int itemNum, qboolean bHadItem ) 
{
	...
	if (bg_itemlist[itemNum].classname && bg_itemlist[itemNum].classname[0])
	{
		...
		if (cgi_SP_GetStringTextString("SP_INGAME_PICKUPLINE",text, sizeof(text)) )
		{
			...
			{
				switch ( bg_itemlist[itemNum].giTag )
				{
				case INV_JETPACK:
				case INV_CLOAK:
				case INV_FORCE_GEM:
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) 
{
	...
	case EV_CHANGE_WEAPON:
		...
		if ( es->weapon == WP_SABER )
		{
			...
			if ( !cent->gent || 
				!cent->gent->client || 
				(cent->currentState.saberInFlight == qfalse && cent->currentState.saberActive == qtrue) )
			{
	...
	case EV_SCEPTER_ALT_SHOT:
		DEBUGNAME("EV_SCEPTER_ALT_SHOT");
		FX_ScepterAltShot( cent->currentState.origin2, cent->lerpOrigin );
		break;

	case EV_SCEPTER_ALT_MISS:
		DEBUGNAME("EV_SCEPTER_ALT_MISS");
		FX_ScepterAltMiss( cent->lerpOrigin, cent->gent->pos1 );
		break;
...
//		break;
	...
	case EV_KOTHOS_BEAM:
		...
		{
			FX_KothosBeam( 
				cg_entities[cent->currentState.otherEntityNum].gent->client->renderInfo.handRPoint, 
		...
		{
			FX_KothosBeam( 
				cg_entities[cent->currentState.otherEntityNum].gent->client->renderInfo.handLPoint, 
				cg_entities[cent->currentState.otherEntityNum2].lerpOrigin 
			);
		}
		break;
...
*/
				...
				if ( cent->gent->owner->client )
				{
					...
					{//play an extra sound
						...
						case CLASS_SENTRY:
							...
							break;
						default://Only the humanoids scream
	...
	case EV_PAIN:
		...
		if ( cent->gent 
			&& cent->gent->NPC 
			&& (cent->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT) )
		{
			return;
		}
...
*/
	...
	case EV_USE_INV_FORCE_GEM:
		DEBUGNAME("EV_USE_INV_FORCE_GEM");
		UseItem(INV_FORCE_GEM);
		break;
