...
void CG_RegisterWeapon( int weaponNum ) 
{
	...
	case WP_SABER:
		...
		cgs.effects.forceLightningWideRed	= theFxScheduler.RegisterEffect( "force/redlightningwide" );
		...
		*/
		...
		cgs.media.forceCoronaLightShader	= cgi_R_RegisterShaderNoMip( "gfx/hud/force_swirl_light" );
		cgs.media.forceCoronaSaberShader	= cgi_R_RegisterShaderNoMip( "gfx/hud/force_swirl_saber" );
		cgs.media.forceCoronaDarkShader		= cgi_R_RegisterShaderNoMip( "gfx/hud/force_swirl_dark" );
		cgs.media.hackerIconShader			= cgi_R_RegisterShaderNoMip( "gfx/mp/c_icon_tech" );
...
//		theFxScheduler.RegisterEffect( "flechette/explosion" );
	...
	case WP_SCEPTER:
		//???
		theFxScheduler.RegisterEffect( "scepter/player/alt_miss" );
		theFxScheduler.RegisterEffect( "scepter/player/alt_hit" );
		break;
...
void CG_AddViewWeapon( playerState_t *ps )
{
	...
	if ( cent->gent 
		&& cent->gent->client 
		&& cent->gent->client->ps.forcePowersActive&(1<<FP_LIGHTNING) )
	{//doing the electrocuting
		...
		{//arc
			...
			if ( cent->gent->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_5 )
			{
				levelEffect = cgs.effects.forceLightningWideRed;
			}

			theFxScheduler.PlayEffect( levelEffect, temp, cg.refdef.viewaxis );
		}
		else
		{//line
...
const char *weaponDesc[WP_MELEE] =
{//13
"SABER_DESC",
...
void CG_DrawDataPadWeaponSelect( void )
{
	...
	else if (cg.DataPadWeaponSelect > WP_MELEE)//13
	{
		cg.DataPadWeaponSelect = WP_MELEE;//13;
	}
	...
	if (weaponSelectI < WP_SABER)//1
	{
		weaponSelectI = WP_MELEE;//13;
	}
	...
	*/
	...
	if (weaponSelectI> WP_MELEE)//13
	{
		weaponSelectI = WP_SABER;//1;
	}
	...
	*/
	...
	if (weaponData[cg.weaponSelect].weaponIcon[0])
	{
		...
		if (!CG_WeaponCheck(cg.weaponSelect))
		{
			CG_DrawPic( 
				x-(bigIconSize/2), 
				(y-((bigIconSize-smallIconSize)/2))+10+yOffset, 
				bigIconSize, 
				bigIconSize, 
...
void CG_NextWeapon_f( void ) {
	...
	*/
	...
	for ( i = 0 ; i <= MAX_PLAYER_WEAPONS ; i++ )
	{
		...
		else if ( cg.weaponSelect == WP_MELEE )//WP_DET_PACK
		{
...
void CG_DPNextWeapon_f( void ) {
	...
	*/
	...
	for ( i = 0 ; i <= MAX_PLAYER_WEAPONS ; i++ )
	{
		...
		else if ( cg.DataPadWeaponSelect == WP_MELEE )//WP_DET_PACK
		{
			cg.DataPadWeaponSelect = FIRST_WEAPON;
		}
		else*/
