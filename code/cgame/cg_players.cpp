...
static void CG_PlayerAnimEventDo( centity_t *cent, animevent_t *animEvent )
{
	...
	case AEV_SABER_SWING:
		...
		{//cheat over to game side and play sound from there...
			WP_SaberSwingSound( 
				cent->gent, 
				animEvent->eventData[AED_SABER_SWING_SABERNUM], 
				(swingType_t)animEvent->eventData[AED_SABER_SWING_TYPE] 
			);
		}
		break;
	case AEV_SABER_SPIN:
...
static void CG_BreathPuffs( centity_t *cent, vec3_t angles, vec3_t origin )
{
	...
	if ( (contents & CONTENTS_WATER) 
		&& ( cg_drawBreath.integer == 1 || cg_drawBreath.integer == 3 ) )
	{
		CG_PlayEffectBolted( 
			"misc/waterbreath", 
			cent->gent->playerModel, 
...
static qboolean CG_PlayerLegsYawFromMovement( centity_t *cent, const vec3_t velocity, float *yaw, float fwdAngle, float swingTolMin, float swingTolMax, qboolean alwaysFace )
{
	...
	if ( cent->gent 
		&& cent->gent->client 
		&& cent->gent->client->ps.forcePowersActive & (1 << FP_SPEED) )
	{//using force speed
...
static void CG_ATSTLegsYaw( centity_t *cent, vec3_t trailingLegsAngles )
{
	...
	if ( moving || 
		legsAnim == BOTH_TURN_LEFT1 || 
		legsAnim == BOTH_TURN_RIGHT1 || 
		fabs(legAngleDiff) > 45 )
...
static void CG_G2ClientSpineAngles( centity_t *cent, vec3_t viewAngles, const vec3_t angles, vec3_t thoracicAngles, vec3_t ulAngles, vec3_t llAngles )
{
	...
	*/
	...
	{//FIXME: no need to do this if legs and torso on are same frame
		...
		if ( cg_motionBoneComp.integer > 2 
			&& cent->gent->rootBone >= 0 
			&& cent->gent->lowerLumbarBone >= 0 )
		{//expensive version
			...
			{//set it up
				...
				gi.G2API_InitGhoul2Model( 
					dummyGhoul2, 
					"models/players/_humanoid/_humanoid.glm", 
					dummyHModel, 
					NULL_HANDLE, 
					NULL_HANDLE, 
					0, 
			...
			gi.G2API_GetBoneAnimIndex( 
				&cent->gent->ghoul2[cent->gent->playerModel], 
	...
	//	if ( cent->gent && !Q_stricmp( "atst", cent->gent->NPC_type ) )
		...
		else
		{
			if ( cg_turnAnims.integer && !in_camera && cent->gent->hipsBone >= 0 )
			{
				...
				const qboolean animatingHips = gi.G2API_GetAnimRangeIndex( 
					&cent->gent->ghoul2[cent->gent->playerModel], 
					cent->gent->hipsBone, 
					&startFrame, 
					&endFrame 
		...
		*/
		...
		else
		{
			...
			if ( ((cent->gent->client->ps.eFlags & EF_FORCE_GRIPPED) || 
				((cent->gent->client->NPC_class == CLASS_BOBAFETT || 
					cent->gent->client->NPC_class == CLASS_ROCKETTROOPER || cent->gent->client->ps.inventory[INV_JETPACK] > 0) &&
				...
				&& cent->gent->client->ps.groundEntityNum == ENTITYNUM_NONE )
			{
				...
				if ( (cent->gent->client->NPC_class == CLASS_BOBAFETT || 
					cent->gent->client->NPC_class == CLASS_ROCKETTROOPER || 
					cent->gent->client->ps.inventory[INV_JETPACK] > 0)
					&& cent->gent->client->moveType == MT_FLYSWIM )
				{
					divFactor = 3.0f;
				}
...
static void CG_BoltedEffects( centity_t *cent, const vec3_t origin, vec3_t tempAngles )
{
		...
		&& cent->gent->client->NPC_class == CLASS_VEHICLE )
	{
			...
			&& Q_irand( 0, 1 ) == 0 )
		{
			...
			CG_PlayEffectIDBolted( 
				pVeh->m_pVehicleInfo->iArmorLowFX, 
				parent->playerModel, 
				parent->crotchBolt, 
				parent->s.number, 
				parent->currentOrigin 
			);
		}
	}
}
...
{
	ent->customShader = cgs.media.sightShell;//cgs.media.forceShell;
	...
	{
		ent->shaderRGBA[0] = 51;//0;
		ent->shaderRGBA[1] = 255;//0;
		ent->shaderRGBA[2] = 51;//255;
	...
	ent->shaderRGBA[1] = 204;//255;
	ent->shaderRGBA[2] = 102;//0;
...
//	{TEST: level 3 identifies friend or foe with color
		...
		*/
		...
		if ( cent->gent )
		{
			if ( cent->gent->client || cent->gent->owner )
			{
				switch ( cent->gent->client->NPC_class )
				{
				case CLASS_ATST:
				...
				case CLASS_SENTRY:
					break;
				case CLASS_VEHICLE:
					switch ( cent->gent->m_pVehicle->m_pVehicleInfo->type )
					{
					case VH_ANIMAL:
					case VH_FLIER:
				...
				case CLASS_GALAKMECH:
					...
					break;
				case CLASS_IMPERIAL:
					if ( !Q_stricmp( "Rax", cent->gent->NPC_type ) )
					{
						ent->shaderRGBA[0] = 255;
						ent->shaderRGBA[1] = 51;
						ent->shaderRGBA[2] = 26;
				...
				case CLASS_MORGANKATARN:
					...
					break;
				case CLASS_REBEL:
					if ( !Q_stricmp( "Chewie", cent->gent->NPC_type ))
					{
						ent->shaderRGBA[0] = 26;
						ent->shaderRGBA[1] = 51;
						ent->shaderRGBA[2] = 255;
				...
				case CLASS_ROCKETTROOPER:
					ent->customShader = cgs.media.sightShell;//because the game counts them as a Force users
				...
				default:
					if ( cent->gent->client->ps.forcePowersKnown )
					{
						...
						switch ( FP_EntityForceSide( cent->gent ) )
						{
			...
			if ( cent->gent->item )
			{
				switch ( cent->gent->item->giType )
				{
				case IT_AMMO:
					if ( cent->gent->item->giTag == AMMO_FORCE )
					{
						ent->customShader = cgs.media.forceShell;
						ent->shaderRGBA[0] = 128;
						ent->shaderRGBA[1] = 128;
					...
					break;
				case IT_HOLDABLE:
					if ( cent->gent->item->giTag == INV_FORCE_GEM )
					{
						ent->customShader = cgs.media.forceShell;
						ent->shaderRGBA[0] = 128;
						ent->shaderRGBA[1] = 128;
...
//	}
	...
	if ( gent->client->ps.powerups[PW_DISRUPTION] )
	{
		...
		if ( cg.time - ent->endTime < 1000 
			&& (cg_timescale.value * cg_timescale.value * Q_flrand(0.0f, 1.0f)) > 0.05f )
		{
			vec3_t fxOrg;
			mdxaBone_t	boltMatrix;
	...
	if ( (powerups & ( 1 << PW_SHOCKED_RED )) )
	{
		int	dif = gent->client->ps.powerups[PW_SHOCKED_RED] - cg.time;
		...
		if ( dif > 0 && Q_flrand(0.0f, 1.0f) > 0.4f )
		{
			...
			if ( rand() & 1 )
			{
				ent->customShader = cgs.media.electricBodyShaderRed;
			}
			else
			{
				ent->customShader = cgs.media.electricBody2ShaderRed;
			}
...
extern vmCvar_t	cg_thirdPersonAlpha;
		...
		{//if NPC, third person, or dead, unless using saber
			...
			*/
			...
			if ( cent->gent->client 
				&& (cent->gent->client->ps.powerups[PW_SHOCKED] > cg.time || 
					cent->gent->client->ps.powerups[PW_SHOCKED_RED] > cg.time ))
			{//being electrocuted
				int	powerupEffect = cgs.media.boltShader;

				if ( cent->gent->client->ps.powerups[PW_SHOCKED_RED] > cg.time )
				{
					powerupEffect = cgs.media.boltShaderRed;
				}

				CG_ForceElectrocution( cent, ent.origin, tempAngles, powerupEffect );
			}
			...
			{//doing the electrocuting
				...
				vec3_t	tAng, fxDir;
				int		lightningLevel = cent->gent->client->ps.forcePowerLevel[FP_LIGHTNING];
				...
				{//arc
					vec3_t	fxAxis[3];
					int		levelEffect = cgs.effects.forceLightningWide;
					
					if ( lightningLevel > FORCE_LEVEL_5 )
					{
						levelEffect = cgs.effects.forceLightningWideRed;
					}
					...
					theFxScheduler.PlayEffect( 
						levelEffect,
						cent->gent->client->renderInfo.handLPoint, 
						fxAxis 
					);
					
					switch ( cent->gent->client->ps.torsoAnim )
					{
					...
					case BOTH_FORCE_2HANDEDLIGHTNING_START:
						...
						theFxScheduler.PlayEffect( 
							levelEffect, 
							cent->gent->client->renderInfo.handRPoint, 
							fxAxis 
						);
