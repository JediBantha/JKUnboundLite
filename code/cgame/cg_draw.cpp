...
static void CG_DrawJetpackFuel( void )
{
	...
	CG_FillRect(
		x+1.0f, 
		y+1.0f+(JPFUELBAR_H-percent), 
		JPFUELBAR_W-1.0f, 
		JPFUELBAR_H-1.0f-(JPFUELBAR_H-percent), 
		aColor
	);
...
static void CG_DrawCloakFuel( void )
{
	...
	if ( player->client->ps.jetpackFuel < 100 )
	...
	CG_FillRect(
		x+1.0f, 
		y+1.0f+(CLFUELBAR_H-percent), 
		CLFUELBAR_W-1.0f, 
		CLFUELBAR_H-1.0f-(CLFUELBAR_H-percent), 
		aColor
	);
...
#define HACKBAR_H			5.0f
#define HACKBAR_W			50.0f
#define HACKBAR_X			295.5f
#define HACKBAR_Y			238.0f
//same routine (at least for now), draw progress of a "hack" or whatever
void CG_DrawHaqrBar( void )
{
	gentity_t	*player = &g_entities[0];
	vec4_t		aColor;
	vec4_t		cColor;
	float		x = HACKBAR_X+((HACKBAR_W/2)-(HACKBAR_W/2));
	float		y = (HACKBAR_Y+HACKBAR_H) + 8.0f;
	float		percent = (((float)player->client->ps.hackingTime - (float)cg.time) / (float)player->client->ps.hackingBaseTime) * HACKBAR_W;

	if (percent > HACKBAR_W ||
		percent < 1.0f)
	{
		return;
	}
...
static void CG_DrawAmmo(const centity_t	*cent,const int xPos,const int yPos)
{
	...
	int	weaponColor = CT_AMMO_DEFAULT;
	...
	case WP_JAWA:
		...
		break;
	case WP_DISRUPTOR:
		weaponColor = CT_AMMO_DISRUPTOR;
		break;
	case WP_BOWCASTER:
	case WP_NOGHRI_STICK:
		weaponColor = CT_AMMO_BOWCASTER;
		break;
	case WP_DEMP2:
		weaponColor = CT_AMMO_DEMP2;
		break;
	case WP_CONCUSSION:
	case WP_RAPID_FIRE_CONC:
		weaponColor = CT_AMMO_CONCUSSION;
		break;
	default:
		break;
	}
	...
	else
	{
		if ( currValue > 0 )
		{
			...
			{
				memcpy(calcColor, colorTable[weaponColor], sizeof(vec4_t));//otherHUDBits[OHB_AMMOAMOUNT].color
			}
		}
		else
		{
	...
	memcpy(calcColor, colorTable[weaponColor], sizeof(vec4_t));//colorTable[CT_WHITE]
	...
	for (i=MAX_HUD_TICS-1;i>=0;i--)
	{
		...
		{
			memcpy(calcColor, colorTable[weaponColor], sizeof(vec4_t));	//ammoTics[i].color
...
================
*/
static void CG_DrawHealth(const centity_t *cent,const int xPos,const int yPos)
{
	int			i;
	qboolean	flash=qfalse;
	vec4_t		calcColor;
	...
	if (cg.healthHUDTotalFlashTime > cg.time )
	{
		flash = qtrue;
		
		if (cg.healthHUDNextFlashTime < cg.time)
		{
			cg.healthHUDNextFlashTime = cg.time + 400;

			if (cg.healthHUDActive)
			{
				cg.healthHUDActive = qfalse;
			}
			else
			{
				cg.healthHUDActive = qtrue;
			}
	...
	{
		cg.healthHUDNextFlashTime = 0;
		cg.healthHUDActive = qtrue;
	}

	inc = (float)  cent->gent->client->ps.stats[STAT_MAX_HEALTH] / MAX_HUD_TICS;
	value = cent->gent->health;
	
	if ( value > cent->gent->client->ps.stats[STAT_MAX_HEALTH] )
	{
		extra = value - cent->gent->client->ps.stats[STAT_MAX_HEALTH];
		value = cent->gent->client->ps.stats[STAT_MAX_HEALTH];
	}
	...
	for (i=MAX_HUD_TICS-1;i>=0;i--)
	{
		...
		{
			memcpy(calcColor, colorTable[CT_WHITE], sizeof(vec4_t));
			percent = 0.75f + (sin( cg.time * 0.005f )*((extra/cent->gent->client->ps.stats[STAT_MAX_HEALTH])*0.25f));
...
================
*/
static void CG_DrawArmor( const centity_t *cent, const int xPos, const int yPos )
{
	int			i;
	qboolean	flash=qfalse;
	vec4_t		calcColor;
	...
	{
		if ( cent->gent->client->ps.stats[STAT_ARMOR] <= 0 )
		{
			return;
		}
	}

	if (cg.armorHUDTotalFlashTime > cg.time )
	{
		flash = qtrue;

		if (cg.armorHUDNextFlashTime < cg.time)
		{
			cg.armorHUDNextFlashTime = cg.time + 400;

			if (cg.armorHUDActive)
			{
				cg.armorHUDActive = qfalse;
			}
			else
			{
				cg.armorHUDActive = qtrue;
			}
	...
	{
		cg.armorHUDNextFlashTime = 0;
		cg.armorHUDActive = qtrue;
	}

	inc = (float)  cent->gent->client->ps.stats[STAT_MAX_HEALTH] / MAX_HUD_TICS;
	value = cent->gent->client->ps.stats[STAT_ARMOR];
	...
	CG_DrawNumField (
		...
		3,
		cent->gent->client->ps.stats[STAT_ARMOR],
...
static void CG_DrawSimpleSaberStyle( const centity_t *cent )
{
	...
	case SS_DESANN:
		...
		calcColor = CT_MAGENTA;//CT_HUD_RED;
		break;
	case SS_TAVION:
		Com_sprintf( num, sizeof( num ), "TAVION" );
		calcColor = CT_ICON_GREEN;//CT_ICON_BLUE;
		break;
	case SS_DUAL:
		Com_sprintf( num, sizeof( num ), "AKIMBO" );
		calcColor = CT_YELLOW;//CT_HUD_ORANGE;
		break;
	case SS_STAFF:
...
static void CG_DrawHUD( centity_t *cent )
{
	...
	if (cgi_UI_GetMenuInfo("lefthud",&sectionXPos,&sectionYPos,&sectionWidth,&sectionHeight))
	{
		...
		CG_DrawArmor( cent, sectionXPos, sectionYPos );

		CG_DrawHealth( cent, sectionXPos, sectionYPos );
	}
...
{
	int	healthBarRange = HEALTHBARRANGE;
	...
	/*
	if ( g_entities[0].client->ps.forcePowerLevel[FP_SEE] > 2 )
	{
		healthBarRange = (384 + (64 * (g_entities[0].client->ps.forcePowerLevel[FP_SEE] - 3)));
		
		if ( healthBarRange > 1024 )
		{
			healthBarRange = 1024;
		}
	}
...
static void CG_DrawCrosshair( vec3_t worldPoint )
{
	...
	else if ( cg_crosshairIdentifyTarget.integer )
	{
		...
		if ( crossEnt->client )
		{
			...
			else if ( g_entities[0].client 
				&& g_entities[0].client->playerTeam == TEAM_FREE )
			...
			{
				switch ( crossEnt->client->playerTeam )
				{
				case TEAM_PLAYER:
					//Allies are green
					ecolor[0] = 0.1f;//R	//0.0f
					ecolor[1] = 1.0f;//G
					ecolor[2] = 0.1f;//B	//0.0f
					break;
				case TEAM_NEUTRAL:
					// NOTE: was yellow, but making it white unless they really decide they want to see colors
					ecolor[0] = 1.0f;//R
					ecolor[1] = 0.8f;//G	//1.0f
					ecolor[2] = 0.2f;//B	//1.0f
					break;
				default:
					//Enemies are red
					ecolor[0] = 1.0f;//R
					ecolor[1] = 0.1f;//G	//0.0f
					ecolor[2] = 0.1f;//B	//0.0f
					break;
				}
			}
...
//-----------------------------------------
{
	...
	if ( !traceEnt || 
		...
		(!traceEnt->client && traceEnt->s.weapon != WP_TURRET ) || 
		!traceEnt->health || 
		( traceEnt 
			&& traceEnt->client 
			&& traceEnt->client->ps.powerups[PW_CLOAKED] ))
	{
...
static void CG_ScanForCrosshairEntity( qboolean scanAll )
{
	...
	Vehicle_t	*pVeh = NULL;
	int			pushLevel = cg_entities[0].gent->client->ps.forcePowerLevel[FP_PUSH];
	int			pullLevel = cg_entities[0].gent->client->ps.forcePowerLevel[FP_PULL];
	...
	{//try to check for force-affectable stuff first
		...
		if ( ( pVeh = G_IsRidingVehicle( cg_entities[0].gent ) ) != NULL 
			&& cg_entities[0].currentState.eFlags & EF_NODRAW )
		{
		...
		{//hit something
			...
			if ( traceEnt )
			{
				...
				if ( traceEnt->client )
				{//is a client
					if ( cg_entities[0].gent->client->ps.forcePowerLevel[FP_TELEPATHY] 
						&& traceEnt->health > 0 
				...
				{//hit a mover
					...
					{//it's a func_static
						...
						{//pushable only
							if ( pushRange >= trace.fraction * 2048 )
						...
						{//pullable only
							if ( pullRange >= trace.fraction * 2048 )
	...
	if ( !cg_forceCrosshair )
	{
		...
		{//100% accurate
			...
			else if ( cg.snap->ps.weapon == WP_NONE || 
...
static void CG_DrawShieldWarning( void )
{
	char		text[1024]={0};
	int			w;
	...
	if ( cg_drawShieldWarning.integer <= 0 ) 
	{
		return;
	}
...
static void CG_Draw2D( void )
{
	...
	{
		if ( !(cent->gent 
			&& cent->gent->s.eFlags & (EF_LOCKED_TO_WEAPON )))//|EF_IN_ATST
		{
